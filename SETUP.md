...
> 같은 Neon 프로젝트를 쓰기로 했다 (근거는 `MILESTONES.md` "확정된 설계 결정" 표).
> **단 브랜치는 나눈다 — 아래 1-2 절.**

### ★ 개발과 운영은 Neon 브랜치로 나눈다 (2026-08-26 ~)

**같은 프로젝트, 다른 브랜치다.** 프로젝트를 나누지 않은 이유는 리전·요금·설정을 두 번
관리하지 않기 위해서고, 브랜치만으로 스키마와 데이터가 갈린다.

| | 브랜치 | 누가 쓰나 |
|---|---|---|
| 운영 | `production` | Vercel 환경변수 (**로컬에서 쓰지 말 것**) |
| 개발 | `dev` | 로컬 `.env` |

**왜 나눴나** (2026-08-26). 원래는 한 DB 였는데 `db push` 가 팀이 쓰는 DB 로 직행했고,
개발 중 테스트 업로드가 팀 문서 목록에 섞였다 — **이미 일어난 일이다.** 2026-08-26 시점
문서 15건 중 8건이 휴지통이었고 상당수가 테스트였다. 영구삭제까지 있는 앱이라 개발 중
실수가 팀 문서에 닿는다.

> **디스코드 알림은 원래부터 안전했다.** `discord.ts:78` 의 `NODE_ENV !== 'production'`
> 게이트 때문에 로컬 dev 에서는 웹훅이 안 나간다. 이건 브랜치 분리와 무관하게 이미 막혀 있다.

> **S3 는 안 나뉜다.** 버킷이 하나라 개발 업로드도 운영 버킷에 객체를 만든다. DB 행이
> 없으니 고아가 될 뿐이지만, `HANDOFF.md` "다음 작업"의 S3 고아 객체 정리와 같은 자리다.

**브랜치 만들기** — Neon 콘솔 → Branches → New Branch. 이름 `dev`, 부모는 운영 브랜치,
**Include data** 로 만들면 운영 데이터 사본이 따라온다 (실파일명으로 테스트할 수 있다).

#### 스키마를 올리는 절차 — 두 번 민다

**`prisma/migrations` 가 없다. 이 리포는 `db push` 만 써 왔다.** 브랜치를 나눈 순간
"한 번 밀면 개발도 운영도 끝"이 아니게 됐으므로 순서를 지켜야 한다.

```bash
# 1) 개발 브랜치에 먼저
DATABASE_URL="<dev 의 direct URL>" npx prisma db push

# 2) 브라우저로 확인한 뒤 운영에
DATABASE_URL="<운영의 direct URL>" npx prisma db push
```

**2번을 빼먹으면 배포된 앱만 터진다** — 로컬은 멀쩡한데 운영에 컬럼이 없어서 런타임에만
죽는다. 타입 검사도 빌드도 이걸 못 잡는다 (`prisma generate` 는 스키마 파일만 본다).
...
동적 클라이언트 등록(DCR)으로 알아서 붙는다. claude.ai 는 설정 → 커넥터 →
커스텀 커넥터 추가이고, 인증은 **지금 로그인**, OAuth 클라이언트는 **자동으로 등록**을
고른다(2026-09-12 기준 둘 다 "감지됨"으로 미리 선택된다 — 우리 401 챌린지와
`registration_endpoint` 를 읽은 결과다). *로그인 없음*을 고르면 토큰이 없어 401 만 받는다.
...
받기로 한 대가다 (`HANDOFF.md` 미룬 항목). refresh 의 30일은 **처음 동의한 날부터**라
리프레시해도 늘어나지 않는다 — 30일마다 한 번 다시 연결(동의)해야 한다.

...
/** `2026_08_17` `2026.08.17` `20260819` `260817`. 버전과 같은 이유로 토큰화 전에 지운다.
    내보낸다(2026-09-22) — `file-version.ts` 의 날짜 파서가 같은 벌을 재사용한다. 정규식을
    두 벌 두면 한쪽 요구로 고칠 때 다른 쪽이 조용히 깨진다(위 VERSION_TOKEN과 같은 이유). */
export const DATE_TOKEN = /(?<=^|[\s_—–-])(?:\d{4}[._-]\d{1,2}[._-]\d{1,2}|\d{8}|\d{6})(?=[\s_—–-]|$)/gu

...
**M0 · M1 · M2 · M3 · M4 완료. M6(배포) 은 5개 중 4개.**
...
**남은 하나는 길드 비멤버 거부 확인이고, 다른 디스코드 계정이 없어 아직 못 했다.**
파이프라인 도구(`orchestrate.sh`) 수정도 끝났다 — 아래 "파이프라인 부검" 절.
**단, M4 주행에서 결함 2건이 새로 나왔고 아직 안 고쳤다** — "파이프라인 결함" 절.

**다음은 M6 의 마지막 항목(길드 비멤버 거부 확인)과 M5(미리보기)다.** 근거는 "다음 작업"에 있다.
...
### 배포 성능 실측 (2026-08-25) — 미조치

배포 후 "접속이 느리다"는 보고가 있어 재 봤다. **원인은 코드가 아니라 지리적 배치다.**
...
`MILESTONES.md` M6 는 싱가포르로 뒀다고 적혀 있으나 **실제로는 적용되지 않았다.**

```
지금:  브라우저(한국) → 엣지(서울) → 함수(미국 동부) → Neon(싱가포르) → 역순
                                      └─ 태평양 횡단 ─┘  └─ 쿼리마다 미국↔싱가포르 ─┘
```

**목록 페이지는 DB 를 3번 친다.** `layout.tsx:28`(폴더 목록) → `page.tsx:47`(폴더 이름,
`?folder=` 일 때만) → `page.tsx:50`(문서 목록). **전부 순차**라 왕복 비용이 그대로 곱해진다.

고칠 순서 (효과 큰 것부터):

1. **함수 리전을 `sin1`(싱가포르)로.** Settings → Functions → Region, 그 뒤 재배포.
   DB 왕복이 200ms 대에서 1ms 대로 떨어지고 쿼리가 3개라 3배로 붙는다.
   서울(`icn1`)이 아니라 싱가포르인 이유: 사용자는 가까워지지만 DB 가 80ms 거리로
   남는다. **쿼리가 여러 개면 DB 옆이 유리하다.**
2. **`page.tsx:47,50` 을 `Promise.all` 로 병렬화.** 폴더 이름 조회와 문서 목록 조회는
   서로 의존하지 않는다. 1번을 먼저 하면 이득이 작아지므로(1ms×2) **1번 이후에
   재측정하고 판단할 것.**
3. **Neon 콜드 스타트** — 무료 티어는 몇 분 유휴면 컴퓨트가 0 으로 내려간다. 위 1230ms
   가 그것이다. 코드로 못 고치고, 팀이 실제로 쓰기 시작하면 줄어든다.
4. **Vercel 콜드 스타트** — 1680ms vs 웜 315ms. 서버리스의 구조적 특성이다.

> 3·4 는 무료 티어의 성질이지 버그가 아니다. **1 을 하기 전에 코드를 고치지 말 것** —
> 지금 숫자는 리전이 잘못 잡힌 상태의 것이라, 그 위에서 최적화하면 엉뚱한 곳을 판다.
...
1. **함수 리전을 `sin1` 로 바꾸고 재측정.** 배포는 됐는데 함수가 `iad1`(미국 동부)에서
   돌고 있다 — 위 "배포 성능 실측" 절. 클릭 몇 번이고 이 목록에서 효과가 가장 크다.

2. **길드 비멤버 거부 확인 (M6 의 마지막 항목).** 다른 디스코드 계정이 필요하다.
   **팀에 URL 을 뿌리기 전에 할 것** — 접근 제어가 길드 멤버십 하나뿐이라 이게 유일한
   방어선이고, 뿌리고 나면 그 URL 은 어디로든 샌다. 그다음 `DISCORD_WEBHOOK_URL` 을
   채워 재배포하면 알림이 켜지고 M6 가 끝난다 (M5 의 알림 항목도 같이 끝난다).

   아래는 이미 끝난 배포 작업의 기록이다. **EC2 가 아니다** (2026-08-25 변경, 근거는
   `MILESTONES.md` "확정된 설계 결정" 표). 두 가지가 겹쳐 바꿨다 — `hymn.pem` 이 이 PC 에
   없어 EC2 에 못 붙고, DB 를 Neon 으로 옮기면서 **앱이 EC2 안에 있어야 할 이유(VPC 안의
   RDS)가 사라졌다.** RDS 에 이어서 쓸 DMS 데이터도 없다(2026-08-25 확인).
   탄력적 IP·PM2·Nginx·Let's Encrypt 항목이 통째로 없어졌고, `DATABASE_URL` 을 RDS 로
   되돌리는 항목도 없어졌다 — 개발도 운영도 같은 Neon 을 쓴다.
   **`DATABASE_URL` 은 Neon 의 pooled 엔드포인트로 넣을 것** — 서버리스는 함수 인스턴스가
   여러 개 뜨고 `prisma.ts:16` 의 `max: 5` 는 인스턴스당이라 곱해진다. 개발용 `db push` 는
   direct 를 그대로 쓴다(pooled 로는 스키마 변경이 어긋난다).
   **`infra/s3-cors.json` 에 배포 주소를 넣는 것을 빠뜨리지 말 것** — 지금은 localhost 뿐이라
   빠뜨리면 업로드가 **조용히** 실패한다(브라우저 → S3 직접 PUT 이라 화면에 이유가 안 나온다).
   **배포하는 순간 디스코드 알림이 켜진다** — `discord.ts:78` 의 `NODE_ENV !== 'production'`
   게이트가 유일한 차단막인데 Vercel 은 production 이다. 테스트 업로드가 팀 채널에 쏟아지지
   않게 `DISCORD_WEBHOOK_URL` 을 잠시 비워 둘 것.
   **체크리스트 0번은 길드 비멤버 거부 확인이다** — 접근 제어가 디스코드 길드 멤버십
   하나뿐이라 이게 유일한 방어선인데 아직 미검증이다 (다른 디스코드 계정 필요).
   URL 을 7명에게 뿌리고 나면 그 URL 은 어디로든 샌다.

3. **M5 — 미리보기 + 디스코드 알림** (추정 2h). **알림 쪽은 코드 작업이 거의 없다** —
...
4. **S3 고아 객체 정리** — 업로드 취소와 소프트 삭제 경로에 `deleteObject` 연결.

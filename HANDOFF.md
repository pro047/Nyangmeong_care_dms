...
전부 뜨고 동작한다. **다만 파괴적 버튼의 빨강이 죽어 있다** — 아래 "실측에서 나온 결함".
...
**브라우저 실측은 끝났다** (2026-08-27). **다음은 0-1(제안 이름 편집)이고, 그다음이
M5(미리보기)다.** 근거는 "다음 작업"에 있다.
...
### 실측에서 나온 결함 (2026-08-27) — 미수정

**파괴적 버튼의 빨강이 죽어 있다.** `폴더 삭제`·`영구삭제` 두 곳이 코드로는
`bg-danger`(`#c50000`) 인데 화면에는 검정(`rgb(23,23,23)`)으로 나온다. 되돌릴 수 없는
동작 두 개가 일반 버튼과 구분이 안 된다.
...
원인은 `alert-dialog.tsx:158-161` 이다. `<Button variant="default" asChild>` 안에서
Radix Slot 이 두 className 을 **문자열로 이어붙이기만** 하고 tailwind-merge 를 안 거친다.
`bg-primary` 와 `bg-danger` 가 둘 다 남고, 클래스 순서가 아니라 **CSS 생성 순서**가
승자를 정한다. 고치는 방향은 `className` 을 Action 이 아니라 **Button 에 넘겨** `cn` 이
충돌 클래스를 떨어뜨리게 하는 것이다.
...
브라우저 실측은 2026-08-27 에 끝났다. 원래 순서는 M4 → M6 → M5 였다.
...
0-2. **파괴적 버튼의 빨강을 되살린다** (2026-08-27 실측에서 나옴). `폴더 삭제`·
   `영구삭제` 가 `bg-danger` 를 지정하고도 검정으로 나온다. `alert-dialog.tsx:158-161` 에서
   `className` 을 Action 이 아니라 **Button 에 넘겨** `cn` 이 충돌 클래스를 떨어뜨리게
   한다. 근거와 계측값은 위 "실측에서 나온 결함" 절. **0-1 보다 싸고, 되돌릴 수 없는
   동작 두 개가 걸려 있다** — 먼저 해도 된다.

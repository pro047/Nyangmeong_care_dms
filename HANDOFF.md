...
**정합성 지표 화면이 운영에 배포됐다** (2026-09-14). 메인 문서 목록 위 밴드에 축 19개와
findings 가 나온다. 저쪽 완료 판정(*"메인에서 축과 findings 가 보인다"*)이 **닫혔고 저쪽에도
알렸다**. 운영 `measured_at` 도 `timestamptz(3)` 로 바꿨다(값 3건 보존 확인). 일지는 아래 절.

**버전 이력 표시도 같이 배포됐다** — 목록에 `v0.3 · 3회` 신호를 달고 눌러 펼치게 했다.
**화면 문구에서 "판"을 전부 뺐다.** 실사용 신고에서 나온 스트림이다. 일지는 아래 절.

**MCP 서버는 3단계까지 배포되고 운영 확인까지 끝났다** (1단계 `c2e658d` 2026-09-12 ·
2단계 `f04dd12` 2026-09-13 · 3단계 `88fa444` + 결함 수정 `c7e1a68` 2026-09-14). 읽기 4개 +
올리기 5개 + **문서 본문을 읽는 `read_document`** 가 운영에서 돈다(B안 — 서버가 텍스트로 싣기).
**claude.ai 웹챗과 Claude Code 둘 다에서 본문이 읽히고**, 운영 xlsx **19건 전부** 열리며, 웹
미리보기도 접두사 xlsx 를 연다(2026-09-14 사람 확인). 전부 워크트리 `pearlfish`(`feature/mcp-read`)
세션이 올린 스트림이다 — 기록은 `MILESTONES.md` §'MCP 서버', 남은 것은 아래 0-3.
...
**다음 스트림은 안 정해졌다.** 후보는 **검색** 하나다(§운영 실측이 근거를 열어 뒀다 — 팀이
쓴다). *이 자리에 ① MCP 3단계가 같이 적혀 있었는데 2026-09-14 에 배포·확인까지 끝나 빠졌다.*
...
**재업로드가 흔해졌다** (2026-09-13 운영 실측). 활성 **33건 중 10건(30%)이 버전 2개 이상**
이고 최근 2주에 새 버전 9건이다. `latest.ts` 주석의 *"28건 중 27건이 versionNo=1"*(3.6%)이
8배 틀어져 있었다 — 붙이기가 쓰이기 시작한 결과다. **"재업로드는 거의 없다"를 전제로 쓴
판단이 있으면 다시 재야 한다.**
...
### 정합성 지표 화면 (2026-09-14 배포) — **닫힘**

메인(`/`) 문서 목록 위 밴드. 카드 셋이 **팀원이 묻는 질문 셋**이다 — `검사가 찾은 것` ·
`문서 사이 연결` · `참고: 아직 안 쓴 것`. 설계 근거와 실측표는 `MILESTONES.md`
§'정합성 지표 표시' · §'대시보드로 다시 그림' · §'새 축 반영' · §'밴드 높이'.

**저쪽 완료 판정이 닫혔고 저쪽에도 알렸다.** 운영 화면에서 09-13 22:54 측정이 그려지는 것을
직접 읽어 확인했다.

**하루에 계약이 세 번 바뀌었다** — 축 9 → 19, findings 149 → 284 → 264. 그때마다 화면이
안 깨진 이유는 **모르는 값을 버리지 않고 원문 그대로 띄우게** 짜 뒀기 때문이다(모르는 축 ·
모르는 `check` · 모르는 `level` 셋 다). 다만 그 약속이 **반만 맞았다** — §아래 참조.
...
> **"모르는 축이 와도 안 깨진다"가 반만 맞았다.** 깨지진 않았지만 **조용히 합쳐졌다** —
> `scrFuncCoverage` 9행이 축 이름 하나로 키가 겹쳤다. 조용히 사라지는 것을 막으려고
> `others` 를 뒀는데 정작 합쳐지는 경로를 남겼다. `axisKey()` 로 고쳤다.

> **착수 지시서의 함정 하나가 실측으로 뒤집혔다.** *"`measuredAt` 을 그대로 찍으면 9시간
> 틀린다"* 를 전제로 `timestamptz` 를 골랐는데, **`timestamp` 로 되돌려도 화면은 맞게
> 나온다** — 쓰는 쪽과 읽는 쪽 TZ 가 같으면 상쇄된다. 타입 변경이 실제로 사는 것은
> **다른 경로로 읽을 때**(`::text`·psql·`toISOString()`)와 **이 컬럼을 키로 쓰는 쿼리**다.
> 후자는 실제로 물렸다 — E2E 정리가 0행을 지우고도 "0건 남음"으로 보고했다.

> **E2E 픽스처가 계약을 못 따라가면 새 코드가 검사 밖에 남는다.** 축이 19개가 됐는데
> 픽스처가 9개인 채여서, **`triangle` 과 문서별 9줄을 지워도 13/13 이 통과**했다(실제로
> 지워 보고 확인). 저쪽이 축을 더하면 `test/e2e/consistency-view.mjs` 의 `METRICS` 도 더할 것.

**화면 구현은 `codex exec` 서브에이전트가 했다** (사람이 정함). 설계는 사람과 세션이 먼저
확정해 `docs/codex-consistency-brief.md` 에 박고 *"그리기만 하라"* 로 넘겼다. **판정은
세션이 한다** — 건드린 파일·커밋 시도·검증·실물 화면을 세션이 직접 봤다. Codex 가 **브리프의
모순**(§0 "3개 파일만" vs §7 "E2E 추가하라")을 임의로 해결하지 않고 보고했다.

### 버전 이력 표시 (2026-09-14 배포) — **닫힘**

**실사용 신고에서 나왔다.** 운영에서 새 버전을 올렸는데 목록이 한 글자도 안 바뀌어
*"이전 문서가 하나도 없다"* 로 읽혔다 — 실제로는 요구사항정의서에 **5회가 쌓여 있었다.**

목록 버전 칸에 `v0.3 · 3회` 를 달고(2회 이상일 때만) 누르면 행 아래로 이력이 펼쳐진다.
회차별 받기는 `?v=N` 이다 — 라우트가 이미 지원하던 것을 화면이 안 쓰고 있었다.

**화면 문구에서 "판"을 전부 뺐다.** 같은 동작을 상세는 `새 버전 올리기`, 업로드는
`새 판으로 붙이기` 로 부르고 있었다. 상세 이력 열 이름도 `버전` → **`회차`** 다 — 목록의
`버전`(파일명 `v0.6`)과 같은 이름으로 다른 숫자를 말하고 있었다.

기록은 `MILESTONES.md` §'버전 이력 표시'.

> **목록의 `버전` 열은 파일명에서 읽은 값이라 재업로드해도 안 움직인다.** 운영 실물에서
> 4회·5회가 둘 다 `v0.6` 이었다. **"목록이 안 바뀌었다"를 "안 올라갔다"로 읽지 말 것.**
...
**살아 있는 코드 작업이 없다** (2026-09-14 기준). 0 번(정합성 지표 화면)과 버전 이력 표시는
배포되어 닫혔고, **0-3(MCP)도 3단계까지 배포·운영 확인이 끝났다**(2026-09-14) — 남은 것은
0-3 의 계기 대기 확인 항목뿐이다.
1~3 은 코드가 아니거나 계기 대기다. M0~M6 과 이 목록의 예전 0~4 번은 전부 끝났고, 그 기록은
`HANDOFF-ARCHIVE.md` 의 "§다음 작업 — 끝난 항목들" 절에 있다.

**다음 스트림 후보는 검색 하나다.** 근거는 열려 있는데(팀이 쓴다 · 문서 35건) **착수 지시가
없다** — 고르면 설계부터다. *이 자리에 MCP 3단계가 후보로 적혀 있었는데 2026-09-14 에
코드가 들어와 빠졌다.*

0. ~~**정합성 지표 화면**~~ — **닫혔다 (2026-09-14 배포).** 저쪽 완료 판정도 닫혔고 알렸다.
   운영 `measured_at` 을 `timestamptz(3)` 로 바꿨고(값 3건 보존 확인) `migrate diff` 가 비었다.
   착수 지시서 전문은 `HANDOFF-ARCHIVE.md` 의 "§정합성 지표 화면 착수 지시서" 절에 있다.

   > **다음 측정이 오면 화면이 자동으로 최신 것을 그린다** (`measuredAt desc`). 저쪽이 축을
   > 더하면 **`test/e2e/consistency-view.mjs` 의 `METRICS` 픽스처도 같이 올릴 것** — 안 올리면
   > 새 코드가 검사 밖에 남는다(실제로 겪었다).

0-1. **정합성 화면에 남은 작은 것 둘** (계기 대기, 급하지 않음).

   - **`scrFuncCoverage` 문서별 숫자의 문구가 잠정이다.** 저쪽이 *"120건 중 진짜 누락이 몇
     건인지는 팀장 판정 대상"* 이라고 했다. 판정이 나오면 `0/4` 를 뭐라고 적을지 다시 본다 —
     지금은 판정 없는 중립 표현이다.
   - **스냅샷 비교 화면은 만들지 않았다.** 만들면 §지뢰의 *"스냅샷 두 개를 나란히 놓으면…"*
     두 함정을 정면으로 밟는다. 만들기 전에 그 절을 읽을 것.
...
*이 문단은 여러 번 낡았다 — 2026-09-11 에 "살아 있는 코드 작업이 없다"로 끝났고, 09-12 에
"0 번이 다음"으로, 09-13 에 "0 번은 코드만 끝났다"로 고쳤다. **지금(2026-09-14)은 0 번도
버전 이력도 배포되어 닫혔다.*** 후보와 그 상태는 이 절 머리말이 정본이다 — 여기서 또
갈리지 않게 **날짜를 붙여 고칠 것.**

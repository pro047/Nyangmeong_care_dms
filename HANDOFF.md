...
**다음은 M4(폴더·태그·검색)이고, 파이프라인으로 돌린다.** 순서 근거는 "다음 작업"에 있다.
...
우선순위 순 (2026-08-24 확정). **M4 → M6 → M5** 다. M3 와 파이프라인 수정은 같은 날 끝났다.

1. **M4 — 폴더 + 태그 + 검색** (추정 4h). **파이프라인으로 돌린다.**
   - **스키마 변경이 없다.** `Folder`(`schema.prisma:25`)·`Tag`(`:81`)·`DocumentTag`(`:89`)
     모델이 이미 다 있다. 마이그레이션도 `db push` 도 필요 없다
   - **폴더는 읽기 경로가 이미 절반 있다.** 목록(`(app)/page.tsx:17,96`)과
     상세(`documents/[id]/page.tsx:17,56`)가 `folder?.name` 을 표시하고,
     `api/documents/route.ts:14,34,58` 이 `folderId` 를 받는다. **없는 건 폴더를 만들고
     문서를 배정하는 쓰기 경로뿐**이라 화면에 늘 `—` 로 뜬다
   - **태그는 앱 코드 사용 0건** (전수 확인). 스키마만 있고 UI·API 전부 미착수다
   - 자리만 잡아둔 `disabled` 두 개를 여기서 살린다 — 헤더 검색창
     (`app-header.tsx:25`), 사이드바 "새 폴더"(`app-sidebar.tsx:50`)
   - **시작 전에 위 지뢰의 "worktree 에는 `.env` 가 안 따라간다" 를 읽을 것**

2. **배포 (M6)** (추정 3h) — 탄력적 IP 할당이 첫 항목. `NODE_ENV=production` 이 PM2에서
   실제로 서는지 확인해야 알림이 나간다. 운영 `.env` 의 `uselibpqcompat=true` 는 개발용
   터널 때문에 필요한 것이므로 그대로 가져가지 말 것 (`SETUP.md` M6 절 참조).

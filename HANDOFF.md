...
> **열린 제안 1건 (사람 결정 대기)**: `GET /api/documents` 응답에 정규화된 판번호 라벨을
> 얹으면 파서가 **한 벌**이 되어 양쪽이 갈릴 수 없다. 공개 API 계약 변경이라 결정 사항이고,
> 그때까지는 저쪽이 정규식을 넓히는 쪽이 맞다.
...
- **`versionNo` 가 높은 쪽이 구판인 실물 사례가 나왔다** (건강기록 화면설계서). 저쪽이 내용을
  ...
  정식 요청이 오면 **어느 것이 앞 판인지 사람이 계획 파일의 `order` 에 명시**해야 한다.
...
- **`04_AI매니저_기능명세서_v0.1` 이 DMS 에 없다.** 통합으로는 안 풀리고 **누가 올려야 한다.**
...
1. **운영 문서 증가분의 출처를 가른다** (관찰 — 이걸로 다음 스트림이 갈린다).
   ...
   | 2026-09-08 | +2건 (요구사항정의서 v0.4·v0.5) | 최신 판정 스트림의 운영 화면. **올린 사람이 `ijinseong2798` = 개발자 본인이다** |

   **2026-09-08 분은 이미 갈렸다 — 개발자다.** 이 2건은 팀 사용의 신호가 아니다.
   아직 모르는 것은 **08-30 → 09-07 증가분 2건**의 출처뿐이고, 그것이 이 관찰의 남은 전부다.
   아래 쿼리는 그 2건을 겨냥해 `created_at` 상한을 `'2026-09-08'` 로 끊어 쓰는 편이 낫다 —
   안 끊으면 오늘 올린 개발자 2건이 섞여 들어와 **"개발자만 쓴다"는 답이 자동으로 나온다.**

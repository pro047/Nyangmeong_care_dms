...
바로 읽어 분석하게 하기** (사람 의향 2026-09-13, 착수 지시는 아래 0-3). 정합성 지표 화면
...
0-3. **MCP 서버 — 1단계 배포됨 · 운영 확인 일부 (2026-09-12).** Claude Code·Codex·claude.ai·
   ...
   **다음 세션 착수 — 3단계: 웹챗에서 문서를 바로 읽기** (사람 의향 2026-09-13).
   막힌 곳은 서버가 아니라 claude.ai 샌드박스가 S3 에 못 붙는 것이다(위 ⓕ). 순서:
   1. **A 부터 한 번 판정한다 (서버 변경 0).** claude.ai 설정의 코드 실행 네트워크 허용 목록에
      `nm-care-989785488374-ap-northeast-2-an.s3.ap-northeast-2.amazonaws.com` 을 넣고, 웹챗에서
      `get_download_url` 을 **새로 받아 곧바로** 읽게 한다. 읽히면 3단계는 코드 없이 끝난다.
      허용 목록을 개인 설정에서 바꿀 수 있는지는 확인하지 않았다(추정 — 플랜·조직 설정에 따라 다를 수 있다)
   2. **A 가 안 되거나 허용 목록을 못 바꾸면 B 를 결정한다 — 게이트 셋이 사람 결정이다.**
      ⓐ 확정 결정 ③("서버 중계 B 는 열지 않는다", `MILESTONES.md` 설계 결정 표)을 **읽기 전용 ·
      작은 파일 예외**로 바꿀지 ⓑ 파일 형식 범위 — html·md·csv 같은 텍스트만이면 서버가
      GetObject 해 텍스트로 돌려주면 되지만, **xlsx 를 넣으면 파서 패키지를 들여야 한다**(패키지
      추가 = 게이트) ⓒ 크기 상한 — Vercel 응답 4.5MB 가 천장이다(`MILESTONES.md:62`)
   3. B 로 가면 "파일은 앱 서버를 거치지 않는다"(`CLAUDE.md`)에 예외가 생긴다 — **`CLAUDE.md` 는
      보호 파일이라 사람이 한 줄 넣는다.**

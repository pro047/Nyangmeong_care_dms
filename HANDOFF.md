...
- **남는 것.** 이미 쪼개진 13묶음 합치기(다음 스트림) · 파일명 계열이 바뀐 것과 다른 폴더에 올린 것
  (약 8/23) · 웹과 `GET /api/documents`·MCP 의 문서 순서가 갈린 것.
...
| D5 ★ | 쪼개진 13묶음 합치기 방식 (운영 데이터 이동, 되돌리기 어려움) | ⓐ 버전 행을 한 문서로 옮기고 번호 재부여 · 빈 문서 삭제 / ⓑ 옛 문서를 휴지통으로만 | 사람이 **ⓐ(진짜 합치기)** 를 원했다(2026-09-22). 세션 제안: **살아남는 문서 = 인계문 §4 의 docKey 문서 id** — 그러면 §4 표와 과거 `ConsistencySnapshotDoc.dmsId` 링크가 안 끊긴다. 버전 순서는 업로드 시각순. **예외: `cmt88cywk…`(꼬리연구소 병행 판) 는 합치지 않는다**(09-10 팀장 결정). SCR-MAN·FN-MAN 의 더 새 문서 2건은 **활성 여부부터 확인**(휴지통 추정) |

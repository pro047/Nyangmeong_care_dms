...
> 운영 DB 에도 PDF·이미지가 없을 것이므로 올려서 봐야 하고, 남기기 싫으면 휴지통 →
> 영구삭제 한다.
...
**같은 날 고쳤다** (0-2) — 아래 "실측에서 나온 결함" 절에 대조표가 있다.
...
실측 진행분은 `MILESTONES.md` "브라우저 실측" 절, 방법은 아래 "실측 방법" 절.
...
0. ~~**브라우저 실측 — 자동 분류 + UI 정비를 한 번에.**~~ **끝났다 (2026-08-27,
   ...
   실행 중 나온 결함은 위 "실측에서 나온 결함" 절. 아래는 그때 세운 계획이고 기록으로 남긴다.
   둘 다 같은 화면(업로드 다이얼로그·
   ...
   - **방법은 위 "실측 방법" 절에 정해 뒀다** — 파이프라인을 쓰지 않고, 하네스를 먼저
...
0-2. ~~**파괴적 버튼의 빨강을 되살린다.**~~ **끝났다 (2026-08-27).** `className` 을
   ...
   `npm run test:e2e:danger`. 대조표와 근거는 위 "실측에서 나온 결함" 절.
...
0-3. ~~**`.claude/settings.json` 에 `PowerShell(...)` 규칙 추가.**~~ **끝났다 (2026-08-27).**
   ...
   권한 거부 8건 중 4건이 이것이다. 근거는 위 "파이프라인 3차 주행 실측".

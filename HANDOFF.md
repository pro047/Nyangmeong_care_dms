...
**업로드가 자동으로 새 버전이 되고 목록이 최근 업로드순이 됐다 — 운영 배포됨** (2026-09-22,
PR #12 → `5e020c3`, Vercel `success`). **성공 지표(쪼개짐이 멈추는가)는 관찰 대기다.** 팀원이 개정할 때마다 새 문서로
올려 운영에 같은 문서가 13묶음 · 56건으로 쪼개져 있었다. 일지는 아래 절, 규칙은 `MILESTONES.md`
§'자동 새 버전 · 최근 업로드'.

**다음 스트림은 정합성 검사 DMS 이관 + docKey + 쪼개진 문서 합치기다** (2026-09-22 사람 결정 —
"이제 여기서 작업한다", 저쪽에 회신은 안 보낸다). 인계문과 이 세션이 대조한 사실은 아래
§'정합성 검사 이관 인계문'. **자동 새 버전을 먼저 한 이유가 이것이다** — 쪼개짐을 안 막고 자동
측정을 켜면 docKey 가 달린 옛 문서를 계속 잰다.
...
~~**다음 스트림은 안 정해졌다.** 후보는 **검색** 하나다~~ — **2026-09-22 에 정해졌다(위).**
검색은 뒤로 밀렸다. *그리고 "검색"은 없는 기능이 아니다* — `src/app/(app)/search/page.tsx` 가
이미 제목·설명·태그로 찾는다(빈 결과 문구가 스스로 그렇게 말한다). 후보였던 것은 그 범위를
넓히는 일이고, 파일 내용 전문 검색은 `CLAUDE.md` 의 범위 밖이다. *이 자리에 ① MCP 3단계가 같이
적혀 있었는데 2026-09-14 에 배포·확인까지 끝나 빠졌다.*
...
### 자동 새 버전 · 최근 업로드 (2026-09-22) — **운영 배포됨 · 관찰 대기**

PR #12 (`feature/auto-attach-recent-upload`, 기능 커밋 `c18d73b`) → 머지 `5e020c3`. Vercel 운영 배포
`success`, 운영 `/login` 200 · 비로그인 페이지 이동 307 → `/login` · 운영 MCP 응답 정상(실측).
**로그인한 운영 화면은 세션이 못 봤다** — 운영 세션을 만들 수 없다. 사람이 확인한다.

> **이 스트림은 아직 닫지 않았다** — 성공 지표가 미측정이라서다. 닫을 때(13묶음이 더 안 늘었다는
> 실측이 나오면) one-in-one-out 으로 아래 09-14 두 절(정합성 지표 화면 · 버전 이력 표시)을
> `HANDOFF-ARCHIVE.md` 로 내린다.

- **무엇이 바뀌나.** 업로드 모달이 같은 문서의 새 판을 알아보면 **묻지 않고 새 버전으로 올린다** —
  더 높은 버전이거나, 같은 버전에 파일명 날짜가 같거나 늦을 때. 후보가 여럿이면 가장 최신 문서에
  붙는다. 나머지는 사람이 고를 때까지 멈춘다. 목록 날짜 열은 `최근 업로드`(최신 버전 업로드 시각)이고
  정렬·구버전 흐림도 같은 값이다. 규칙 표는 `MILESTONES.md` §'자동 새 버전 · 최근 업로드'.
- **왜.** 사람이 확인했다 — 팀원은 선택칸의 뜻을 **몰라서** 기본값(새 문서)으로 올렸다. 운영 업로드
  66건 재생: 옛 규칙 최종 문서 52건 → 새 규칙 24건(실측).
- **검증.** vitest 979 · tsc · lint · build 통과. `npm run test:e2e:attach` **14/14**(dev 3002, 이
  스트림에서 통째로 다시 썼다 — 옛 판은 "항상 먼저 묻는다"를 검사했다). `/code-review` 1건(자동 분류
  모드에서 미리보기와 시작 시점이 다른 폴더로 후보를 봤다) — 고쳤고, 되돌리면 E2E A11 이 실패하는 것까지 봤다.
- **배포 후 볼 것.** 팀원이 실제로 올렸을 때 **13묶음이 더 늘지 않는가**가 이 작업의 성공 지표다.
  운영 MCP `search_documents` 로 폴더별 같은 키 문서 수를 다시 세면 된다.
- **쪼개진 문서 9묶음(25건)을 합쳤다 — 운영 적용** (2026-09-22). 남는 문서는 정합성 인계문 §4 의 docKey id 다.
  활성 77 → 52, 버전 행 127 그대로. 상세·검증은 `MILESTONES.md` §'쪼개진 문서 합치기'.
  **롤백**: `DATABASE_URL=<운영> node scripts/merge-rollback.mjs --snapshot runs/merge-prod-20260922.json --apply`
  (남은 9문서에 새 버전·제목·태그 변경이 생기면 그 그룹은 거절한다) · 또는 적용 전에 만든 Neon 백업 브랜치.
  **없어진 25건을 가리키던 디스코드 알림 링크는 404 다.**
- **남는 것.** 합치지 못한 4묶음(건강기록 화면·기능 · 로그인 화면설계서 — 남길 §4 문서가 묶음 밖 / 로그인 와이어프레임
  — 병행 판) · 파일명 계열이 바뀐 것과 다른 폴더에 올린 것(약 8/23) · 웹과 `GET /api/documents`·MCP 의 문서 순서가 갈린 것.

> **이 스트림을 소넷 서브에이전트가 두 번 나눠 구현했고, 세션이 검토하다 구멍을 하나 막았다.**
> 후보 목록이 모달을 닫을 때만 갱신되므로(`router.refresh()` 가 `close()` 에만 있다), 같은 모달에서
> v0.8 을 올린 뒤 v0.7 을 담으면 **옛 후보와 비교해 자동으로 붙을 뻔했다** — 붙은 판은 뗄 수 없다.
> 같은 모달에서 이미 올린 파일도 "같은 묶음"으로 세게 했다. **서브에이전트 보고의 테스트 통과만
> 보고 넘어갔으면 못 봤다** — 컴포넌트 안 로직이라 vitest 가 덮지 못하는 자리다.

### 정합성 검사 이관 인계문 (2026-09-22 수신) — **다음 스트림 · 착수 전**

저쪽 인계문: `~/orca/Nyangmeong_care/claude/0922_DMS_정합성검사_이관_인계문_v0.1.md` (214줄, 자기완결).
요지: 업로드 때 DMS 가 19개 docKey 최신판 전체를 다시 재서 대시보드에 반영한다 · manifest 폐지 ·
docKey 는 DMS 문서 필드(새 문서만 팀장이 한 번, 붙이기는 이어받음) · 판정은 계속 사람.
골든 출력 `~/orca/Nyangmeong_care/runs/2026-09-22_1/verify.json`(git 밖), 사양 정본 `scripts/docs.py`.

**회신은 안 보냈다** — 사람이 "이제 여기서 작업한다"고 정했다.

#### 착수 전에 정할 것 (2026-09-22 기준 **전부 미결정**)

**★ 는 되돌리기 어려운 결정이라 사람이 정한다**(스키마 · 보호 조항 · 패키지 · 운영 데이터 이동).
정해지기 전에는 코드를 쓰지 않는다. 세션 권고는 근거와 함께 적었다 — 결정이 아니다.

| # | 정할 것 | 선택지 | 세션 권고와 근거 |
|---|---|---|---|
| D1 ★ | 서버가 S3 본문을 읽고 html 을 파싱하는 범위를 **정합성 측정까지 넓히는가** (인계문 §2 #1·#2 — 안 되면 인계문 전체가 멈추고 대안은 GitHub Actions + `repository_dispatch`) | 넓힌다 / GitHub Actions 로 간다 | **넓힌다.** 이미 MCP 가 같은 일을 한다(`getObjectBytes` `src/lib/s3.ts:63` · `htmlToText` `src/lib/mcp/html-text.ts:28`, 코드 확인). 파싱 결과는 문자열로만 나가고 `src/` 에 `dangerouslySetInnerHTML` 0건이라 iframe 격리를 우회하는 경로가 없다. **`CLAUDE.md` "읽기는 예외다" 조항과 `schema.prisma:111` 주석을 같이 고쳐야 한다** — 보호 조항 변경 |
| D2 ★ | docKey 를 어디에 두나 | `documents` 컬럼 + `@unique` / 태그 | **컬럼 + `@unique`** (스키마 변경). 태그는 한 태그에 문서가 여럿 붙는 구조(`DocumentTag`)라 "한 docKey 에 문서 하나"를 DB 가 못 보장한다. 인계문 조건이 유일성이다 |
| D3 ★ | docKey 를 **누가** 달고 바꾸나 (인계문 §3 D "팀장이 단다") | 전원 동등 / `ADMIN_DISCORD_ID` 만 | 사람 판단. "팀장만"은 역할 개념이라 `CLAUDE.md` 의 접근 제어 조항(예외는 삭제 3종·새 버전 올리기뿐)을 **또 넓히는** 일이다. 전원 동등이면 조항은 그대로지만 누구나 docKey 를 옮길 수 있다 |
| D4 ★ | html 파서 패키지 | `htmlparser2` · `parse5` / `jsdom` / 정규식(`html-text.ts` 방식) | 앞의 둘 중 하나(패키지 추가). 스크립트를 실행하지 않고 가볍다. `jsdom` 은 무겁고 설정에 따라 스크립트를 돈다. 정규식은 인계문 §5-3 이 경고한 대로 **표를 평탄화하면 열이 밀린다**(초안이 316건 중 117건 오독) — 표를 태그 트리로 읽어야 한다 |
| ~~D5 ★~~ **9묶음 끝남 (2026-09-22)** | 쪼개진 13묶음 합치기 방식 (운영 데이터 이동, 되돌리기 어려움) — **ⓐ 로 9묶음 운영 적용**(위 §자동 새 버전). 남은 4묶음은 남길 문서를 사람이 정해야 한다 | ⓐ 버전 행을 한 문서로 옮기고 번호 재부여 · 빈 문서 삭제 / ⓑ 옛 문서를 휴지통으로만 | 사람이 **ⓐ(진짜 합치기)** 를 원했다(2026-09-22). 세션 제안: **살아남는 문서 = 인계문 §4 의 docKey 문서 id** — 그러면 §4 표와 과거 `ConsistencySnapshotDoc.dmsId` 링크가 안 끊긴다. 버전 순서는 업로드 시각순. **예외: `cmt88cywk…`(꼬리연구소 병행 판) 는 합치지 않는다**(09-10 팀장 결정). SCR-MAN·FN-MAN 의 더 새 문서 2건은 **활성 여부부터 확인**(휴지통 추정) — *확인됨: 활성이었다. SCR-MAN 은 합쳤고, FN-MAN 은 파일명 계열(`기능_명세서`↔`기능명세서`)이 달라 안 묶인다* |
| D6 | 측정 트리거 방식 | 업로드 응답 전(동기) / 응답 뒤(`after()`) · 디바운스 여부 | **응답 뒤.** 인계문 조건이 "측정 실패가 업로드 실패가 되면 안 된다"다. Next 16 `after()` 가 후보 — **쓰기 전에 `node_modules/next/dist/docs/01-app/03-api-reference/04-functions/after.md` 를 읽을 것**(CLAUDE.md 규칙). 트리거는 라우트가 아니라 `upload-commit.ts` 의 `createDocument`·`addVersion` 에 건다(MCP 경로 포함). 연달아 올라오면 `@@unique([measuredAt])` 가 ms 단위로 부딪힐 수 있다 |
| D7 | Vercel 함수 한도 | — | **사람이 대시보드에서 확인.** `vercel.json` 은 `icn1` 뿐이고 `maxDuration` 설정이 없다. 요금제(Hobby 로 기록돼 있다 — 메모리)의 실행 시간 한도를 세션이 확인 못 했다. 저쪽 실측은 19문서 0.78초(Python, 로컬) · 입력 1.1MB 이하 |

미룰 수 있는 것: 정의 ID 조회 경로(인계문 §5-6, 필수 아님 — 미루기 권고) · 기준 REQ 를 `제.개정내역`
에서 읽는 방법(인계문 §5-5 검사 4, 저쪽도 미검증 — 구현 중 실측) · 행동 사양 90 케이스를 옮길지.

**인계문이 DMS 쪽 사실과 다른 곳** (§10-5, 코드 확인):
- §2 #1 "서버는 본문을 한 번도 안 읽는다 — 그대로다" → **틀림.** 위 1번
- §2 #2 → 서버는 이미 html 을 텍스트로 바꾼다. `dangerouslySetInnerHTML` 은 `src/` 에 0건이라 되그려지는
  경로는 없다. `prisma/schema.prisma:111` 주석은 아직 "DMS 는 계산하지 않는다"
- §7 트리거 지점은 2개가 아니라 **3개** — MCP `create_document`·`add_version` 도 `upload-commit.ts` 를
  탄다. 트리거는 라우트가 아니라 거기에 건다
- §3 D "팀장이 단다"는 **권한 개념**이다 — `CLAUDE.md` 는 역할 추가 금지, 태그는 전원 동등
- xlsx 는 `loadXlsx` 를 거쳐야 한다(§지뢰) — 인계문에 없다
- Vercel `icn1` · `maxDuration` 설정 없음 · 함수 한도는 요금제 의존 — **양쪽 다 미확인**

> **2026-09-22 합치기 뒤: 19개 중 13개가 최신판을 가진 문서를 가리킨다**(제자리 4 + 합침 9). 남은 6개 — SCR-ACC ·
> SCR-HLT · FN-HLT · FN-MAN · FN-PLC · FN-COM. 아래 문단은 합치기 전 기록이다.

**가장 무거운 발견 — §4 docKey 표 19개 중 13개가 그 문서 밖에 더 새 판을 두고 있다** (운영 실측
2026-09-22). 제자리를 지킨 4개(REQ·SCR-COM·SCR-AIM·FN-AIM)는 전부 개발자 계정이 붙이기로 올린
문서다. SCR-MAN·FN-MAN 2개는 더 새 문서가 단건 조회로는 보이는데 검색에는 안 나와 **활성 여부
미확인**(휴지통 추정). 그래서 docKey 를 심기 전에 **합치기(13묶음)** 가 먼저 필요하고, 사람이 "docKey
작업과 같이 한다"고 정했다. **합치기 함정**: `06_로그인_회원가입_와이어프레임` 2건 중 `cmt88cywk…` 는
중복이 아니라 꼬리연구소 병행 판이다(09-10 팀장 결정 — 합치지 않는다). 키만 보고 기계적으로
합치면 깨진다.
...
**살아 있는 코드 작업이 없다** (2026-09-22). 자동 새 버전 · 최근 업로드는 배포됐고(`5e020c3`) 관찰만
남았다. **다음은 정합성 검사 이관 + docKey + 합치기인데, 착수 전에 사람이 정할 것이 7개 있다** —
위 §'정합성 검사 이관 인계문' 의 *착수 전에 정할 것*. 아래 문단은 2026-09-14 기준이다.
...
~~**다음 스트림 후보는 검색 하나다.**~~ **2026-09-22 에 정합성 이관으로 정해졌다(위).** 근거는 열려 있는데(팀이 쓴다 · 문서 35건 — *2026-09-22 에는 `take=50` 상한을 넘었다*) **착수 지시가

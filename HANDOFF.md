...
읽기 4개 + 올리기 5개가 운영에서 돈다 — 요구사항정의서 v5 · AI매니저/공통레이아웃 화면설계서
v3 을 **실제로 MCP 로 올렸다**. 다음은 **3단계(claude.ai 웹챗에서 문서 읽기)** 이고
**B안(서버가 텍스트로 싣기)으로 확정**됐다(사람 결정 2026-09-13). 착수 지시는 아래 0-3.
다른 세션이 올린 스트림이다.
...
**살아 있는 코드 작업은 0-3(MCP 3단계) 하나다.** 0 번(정합성 지표 화면)과 버전 이력 표시는
2026-09-14 에 배포되어 닫혔다 — 일지는 §현재 위치 아래, 기록은 `MILESTONES.md`.
1~3 은 코드가 아니거나 계기 대기다. M0~M6 과 이 목록의 예전 0~4 번은 전부 끝났고, 그 기록은
`HANDOFF-ARCHIVE.md` 의 "§다음 작업 — 끝난 항목들" 절에 있다.

**다음 스트림을 고를 때 후보 둘**: ① **MCP 3단계** — 착수 지시가 아래 0-3 에 있어 바로 뜬다
② **검색** — 근거는 열려 있는데(팀이 쓴다 · 문서 35건) **착수 지시가 없다.** 검색을 고르면
설계부터다.

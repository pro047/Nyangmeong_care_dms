...
- **M0(기반) · M1(로그인·앱 셸) 완료. M2(업로드+목록)는 코드 완성, 실제 검증 대기.**
- **아직 DB에 한 번도 연결한 적이 없다.** 마이그레이션 미실행, 실제 로그인·업로드 미검증.
- 검증된 것: `next build`·`tsc`·`eslint` 통과, `/login` 렌더, 비로그인 시 `/` → `/login` 리다이렉트,
  모든 보호 API 401 응답.
- **미검증: 실제 로그인, S3 업로드, DB 저장.** 자격증명을 채우고 파일 하나를 끝까지
  올려보는 것이 다음 세션의 첫 작업이다.

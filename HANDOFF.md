...
## 현재 위치 (2026-08-28 · 2026-09-01 추가)

> **⚠ 지금 열려 있는 작업 — 폴더 2뎁스, 그리고 그것이 드러낸 후속 하나.**
> worktree `../dms-folder-depth2`(`feature/folder-depth2`)에서 **브라우저 실측 26/26 ·
> dev 소급 이동 apply 까지 끝났다**(2026-09-04). **main 머지는 안 했다.**
>
> **머지를 미루는 이유가 있다.** 소급 이동 뒤 카테고리 폴더를 열면 **빈 화면**이 나온다 —
> `화면설계서` 는 문서 0건이고 16건이 전부 하위에 있는데 목록이 자손을 안 보여주고
> 본문에 자식 폴더도 안 그린다. 원인은 `src/lib/search.ts:30` 의 완전일치 필터이고,
...
### 폴더 2뎁스 자동 분류 스트림 (2026-09-01 착수 · 2026-09-02 검증 통과) — **실측 남음**

`로그인_화면설계서` 를 `화면설계서 > 로그인` 으로 넣는다. 1뎁스로는 폴더가 목록을 못
좁혀서다 — dev 실측에서 `화면설계서` 한 폴더에 11건이 쌓였다.
**사양과 진행 상황의 정본은 `MILESTONES.md` §'폴더 2뎁스 자동 분류'** (확정 규칙 7개·
판정 기준선 26행·소급 이동 규약·단계별 표). 여기에는 그것을 반복하지 않고
**지금 무엇을 해야 하는지**만 적는다.

**파이프라인은 끝났다.** design·judge·impl·verify 전부 DONE, `npm test` 26파일 **385건**·
`lint`·`build` 통과. 커밋 4개가 `feature/folder-depth2` 에 쌓여 있고 워크트리는 깨끗하다.

> **"검증 통과"와 "동작 확인"은 다르다.** `npm test` 는 순수 함수만 지난다 —
> `upload-dialog.tsx` 는 **한 줄도 실행되지 않았다.** judge #29: `ClassifyFolder` 에
> `parentId` 를 넣어도 호출부가 이미 `FolderAliasRow` 를 넘겨 **타입 오류가 안 난다.**
> 경로 라벨·`createFolder(parentId, name)`·흡수 판정은 **컴파일러가 침묵하는 구간**이다.
> **소급 스크립트는 한 번도 실행된 적이 없다.**

**다음에 할 일 (순서대로 — 의존성이 있다)**

0. **사람이 UI 에서 선행 작업 2개** — ① `화면설계서` 에 별칭 `와이어프레임` 추가
   ② 합성 폴더 `로그인 회원가입 와이어프레임`·`플레이스 와이어프레임` 삭제.
   소급 이동의 전제이고 **B9 실측도 여기에 걸린다** (근거·순환 문제는
   `MILESTONES.md` §'소급 이동')
1. **브라우저 B1~B10** — 체크리스트는 워크트리의 `.pipeline/folder-depth2/VERIFY.md` §4.
   dev 서버는 **워크트리에서** 띄운다: `cd ../dms-folder-depth2 && npm run dev` (포트 3002).
   **B1·B4·B5 가 핵심** — 위 인용 블록이 말한 컴파일러가 침묵하는 그 셋이다.
   B10 은 "파일이 앱 서버를 거치지 않는다" 불변식 회귀 (네트워크 탭에서 S3 PUT 직접 확인)
2. **소급 스크립트 S1~S6** (dev DB). `npm run reclassify` → 표 대조 → `-- --apply` → 롤백까지.
   **S2 에서 제안된 하위 폴더 이름의 적절성을 사람이 본다** — 문자열 테스트로는 못 잡는다
3. main 머지 → 배포 → **S7 운영 소급 이동은 사람이 직접**

**`VERIFY.md` §4 의 S0 은 건너뛴다** — `node -p "process.features.typescript"` = `strip`
(2026-09-01 실측, v24.10.0). `--experimental-strip-types` 불필요. verify 세션이 이 문서를
안 읽어 미확인으로 올린 항목이다.

**실측 전에 머지하지 않는다.** 자동 분류 때 실측 전에 머지가 나간 전례가 이 문서에 있고,
이번엔 되돌리기가 더 비싸다 — 소급 스크립트가 팀 문서의 `folder_id` 를 통째로 바꾼다.

**파이프라인 기본 모델이 fable 5.1 로 올라갔다** (`0ba90b1`). 2026-09-01 에 **Fable 한도
소진으로 design 이 16초 만에 죽은** 적이 있는데, 그 경로는 `--fallback-model` 이 못 받아낸다
— 계정 한도는 API 오류가 아니라 CLI 가 메시지를 찍고 종료하는 경로라 **스트림에 `result`
이벤트가 아예 없다**(뒤따르는 `jq: parse error` 는 같은 사건의 증상이다). 그때는
`MODEL_*=claude-opus-5` 로 오버라이드해 우회한다.

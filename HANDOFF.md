...
**단 M6 의 완료 기준("팀원 6명이 각자 접속해 로그인")은 아직 확인 안 됐다** —
URL 은 이미 팀에 배포했고, 실제로 몇 명이 로그인했는지는 `users` 테이블을 세야 안다.
...
> **길드 비멤버 거부는 확인됐다** (2026-08-25, 부계정 실측 — 위 "현재 위치"). M1 부터
> 미검증으로 끌려온 항목이었다. **다만 URL 은 확인 전에 이미 팀에 배포됐다** — 순서가
> 뒤집혔고, 결과가 통과였으니 사고는 안 났다. 다음에 방어선을 검증할 때는 순서를 지킬 것.

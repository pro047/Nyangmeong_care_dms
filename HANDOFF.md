...
**0-1(제안 이름 편집)은 코드가 끝났고 실측만 남았다** (아래 블록). 그다음이 M5(미리보기)다.

> **다음 세션은 여기서 시작한다 — 0-1 의 브라우저 실측 B1~B9 다. 코드는 이미 있다.**
> 파이프라인 4차 주행(`suggest-name-edit`)이 2026-08-27 에 **완주**했고
> `feature/suggest-name-edit` 커밋 `8e3ef4b` 로 **origin 에 푸시돼 있다.**
> **`main` 에는 안 넣었다** — 실측 전에 운영에 내보내지 않기로 한 것이고, 자동 분류
> 때 순서가 뒤집힌 것과 같은 자리를 피한 것이다. `origin/main` 은 `f57c242` 그대로다.
> `npm test` 는 224 → **240**, `lint`·`build` 통과. **그런데 그게 이 변경을 보증하지
> 않는다** — 판단검증 #29 가 짚었듯 기존 e2e 는 `upload-dialog.tsx` 를 지나지 않아
> **이 컴포넌트의 자동 회귀망은 0이다.** 240개는 순수 함수 쪽만 말한다.
>
> 실측 절차와 B1~B9 는 worktree(`C:/Users/ksmart/dms-suggest-name-edit`)의
> `.pipeline/suggest-name-edit/VERIFY.md` §5 에 재현 절차로 적혀 있다.
> 설계는 같은 폴더 `DESIGN.md`, 설계 감사는 `JUDGE.md`.
> 실측은 **로컬 3002(dev 브랜치)에서** 한다 — 브랜치 푸시로 잡히는 Vercel 프리뷰는
> **운영 DB 를 보므로** 거기서 업로드하면 팀 문서에 섞인다.
>
> **실측 전에 조정할 것 둘** (dev DB 상태가 설계 전제와 다르다 — 아래 "dev 브랜치가
> 사라졌다" 절):
> - 제안 그룹을 보려면 **기존 폴더 5개 어디에도 안 걸리는 파일명**이 필요하다
>   (`07_회의록_2026_08_27.html` 같은 것). `와이어프레임` 이 든 파일명은 이제
>   기존 폴더에 부분 일치로 붙는다
> - **별칭이 전부 비어 있다.** B3(기존 폴더 흡수)은 별칭 대신 이름(`화면설계서`)으로
>   확인한다. 별칭 경로까지 보려면 사이드바에서 먼저 별칭을 넣어야 한다
>
> 통과하면 `main` 머지 → 푸시(운영 재배포) → `MILESTONES.md`·이 문서 갱신 순이다.
...
| 맥 NFD 파일명 실분류 | 테스트로 덮었고(`classify.test.ts`) 2026-08-27 에 Windows 산 NFD 파일명으로 **프록시 실측**까지 했다. 남은 것은 **맥 실기** — 맥 팀원이 올린 한글 파일명 1건 |
...
우선순위 순. **0-1 → M5** 가 남았다. M4 는 2026-08-25 에, M6 은 2026-08-26 에,
브라우저 실측과 0-2(파괴적 버튼 빨강) · 0-3(PowerShell 권한)은 2026-08-27 에 끝났다.
원래 순서는 M4 → M6 → M5 였다.
...
0-1. **자동 분류의 제안 폴더 이름을 고친다** — **코드는 끝났다. 브라우저 실측 B1~B9 만
   남았다** (2026-08-27, 파이프라인 4차 주행 `suggest-name-edit` 완주 · 커밋 `8e3ef4b` ·
   `origin/feature/suggest-name-edit` 에 푸시됨 · **`main` 에는 안 넣었다**).
   `npm test` 224 → 240, `lint`·`build` 통과. 절차는 위 "다음 세션은 여기서 시작한다"
   블록과 worktree 의 `.pipeline/suggest-name-edit/VERIFY.md` §5.

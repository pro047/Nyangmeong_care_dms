...
> 전부 빈 Neon DB 에 직접 만든 데이터 위에서 이뤄졌다. M6 에서 RDS 로 되돌릴 때
> 마이그레이션을 다시 적용해야 한다.
...
1. **배포 (M6)** (추정 3h) — 탄력적 IP 할당이 첫 항목. `NODE_ENV=production` 이 PM2에서
   실제로 서는지 확인해야 알림이 나간다. 운영 `.env` 의 `uselibpqcompat=true` 는 개발용
   터널 때문에 필요한 것이므로 그대로 가져가지 말 것 (`SETUP.md` M6 절 참조).
   **개발 `.env` 가 Neon 을 보고 있다** (2026-08-25 전환, 위 "현재 위치" 참조) — 운영은
   RDS 직결이므로 `DATABASE_URL` 을 바꾸고 마이그레이션을 다시 적용해야 한다. Neon 에
   쌓인 M4 실측 데이터는 옮기지 않는다 (테스트 데이터다).

...
**운영 스키마 반영은 아직이다** — `@@unique([s3Key])` 를 미는 `prisma db push` 가 권한
규칙에 막혀 **사람이 직접 돌려야 한다.** direct 엔드포인트로:
`DATABASE_URL="<direct>" npx prisma db push --accept-data-loss`
`migrate diff` 로 확인한 차이는 **정확히 그 인덱스 한 줄**이고, 운영 `s3_key` 중복 0건 ·
버전 행 48 · 고유 48 · NULL 0 이라 실패할 조건이 없다.
...
**다음 스트림은 관찰 결과로 고른다.** 1 번은 2026-09-10 에 닫혔고(팀이 쓴다),
2 번은 관찰이 아니라 **작업으로 승격됐다**(운영에 제약이 없는 것이 확정). 3 은 계기 대기다.
...
2. **운영에 `@@unique([s3Key])` 를 민다** — ~~있는지 확인한다~~ **없는 것이 실측으로 확정됐다
   (2026-09-10). 이건 이제 관찰이 아니라 작업이다.**

   ```
   document_versions 인덱스 (운영, 2026-09-10 실측)
     document_versions_pkey                              (id)
     document_versions_document_id_version_no_key UNIQUE (document_id, version_no)
     document_versions_document_id_version_no_idx        (document_id, version_no DESC)
     ← s3_key 유일 인덱스가 없다
   ```

   > **이 문서의 추정이 틀렸다.** 원문은 *"2026-09-06 S7 운영 `--apply` 가 `Folder.parentId` 를
   > 요구했고 `db push` 는 스키마 전체를 미므로 같이 들어갔을 가능성이 높다"* 였다. 실제로는
   > `folders_parent_id_name_key` 는 있고 `s3_key` 유일 제약만 없다 — 그 제약은 **2026-08-30
   > `36cd4a1` 에 스키마에 들어갔고 운영에는 한 번도 안 밀렸다.** 추정을 근거로 안 민 채
   > 넘어갔으면 그대로 남았을 것이다.

   **무엇이 비는가**: `keyToken` 은 검증돼도 소모되지 않아 5분간 재사용된다. 앱 레벨 검사가
   두 라우트에 있지만(`documents/route.ts`·`[id]/versions/route.ts` 의 `findFirst({where:{s3Key}})`)
   **동시 요청 경합을 막는 것은 DB 제약이었다.** dev 에서는 동시 두 발이 `201/400` 으로
   갈리는 것을 실측했는데(X9), **운영에는 그 방어선이 없다.** 앱이 안 죽으므로 신호도 없다.

   **지금 밀 수 있다 — 운영 `s3_key` 중복 0건을 확인했다** (2026-09-10). 다만 **direct
   엔드포인트가 필요하다**; 오늘 받은 문자열은 `-pooler` 라 스키마 변경에는 못 쓴다.
   `DATABASE_URL="<direct>" npx prisma db push --accept-data-loss`
   **스키마 변경이라 사람 결정 사항이다.**

...
**착수한 스트림이 하나 있고 절반만 끝났다.** 아래 "다음 작업"의 **0번(붙이기 기능)이
지금 유일한 계획된 작업**이다. M0~M6 은 2026-08-28 에 닫혔고, 그 뒤에 연 스트림 여덟
(xlsx·html 미리보기 · S3 고아 정리 · 폴더 2뎁스 · 구버전 흐리기 · 목록 버전 열 ·
권한 경계 · 최신 판정 컬럼 · **목록 API·버전 판정·소급 합치기**)은 전부 배포까지 끝났다.
각 일지는 `HANDOFF-ARCHIVE.md` 에 있다(마지막 것만 아래에 남긴다).

**0번 말고는 여전히 관찰 단계다.** 이 문서와 `MILESTONES.md` 가 여러 곳에서 같은 판단을
내려 뒀다 — 성능도 검색도 *팀이 실제로 쓰는 것을 보고* 정한다. **0번 다음 스트림을 고르기
...
### 목록 API · 버전 판정 · 소급 합치기 (2026-09-09) — 배포·운영 적용까지 끝, **기능은 미완**

세 덩어리가 한 흐름이다. 사내 정합성 저장소(`~/orca/Nyangmeong_care`)가 DMS 최신본을
자동 수급하려다 `405` 로 막힌 것에서 시작해, **왜 최신본을 못 고르는가**로 내려갔다.

**① `GET /api/documents` 를 열었다** (`6fcf492`, 배포됨). 읽기 전용. 활성 문서 전량 +
문서별 최신 `versionNo` 1건. **판정하지 않는다** — 누가 최신본인지는 소비자가 정한다
(근거는 `MILESTONES.md` 의 "목록 API 의 최신 판정" 행). **페이지네이션 금지**가 계약이다.
계약 회귀 테스트 5건이 값이 아니라 관계를 고정한다.

**② `versionNo` 축이 죽어 있었다** (실측). 운영 34건 중 33건이 `versionNo=1` 이었다 —
팀이 `v0.3 → v0.6` 을 재업로드가 아니라 **별개 `Document`** 로 올려서다. 재업로드는 상세
페이지에만 있고 메인 업로드는 **원리상 항상 새 문서를 만든다**(`api/documents/route.ts` 의
`versionNo: 1` 고정).

**③ 판정 함수를 만들었다** (`a316086`). `similar-document.ts`(같은 문서인지) ·
`file-version.ts`(판번호 비교). 순수 함수 + 테스트 33건. 운영 실데이터로 **오탐 0**.
**아직 아무 데서도 안 부른다 — 앱 동작은 안 바뀐다.**

**④ 소급 합치기를 운영에 적용했다** (`1d7e539`·`11f968f`). 7그룹, **활성 문서 34 → 25**,
버전 행 46 유지(파일 손실 0), 고아 0. `versionNo` 2 이상이 1건 → **7건**. 스냅샷은
`runs/merge-prod-20260909.json` — **되돌릴 수 없는 작업이라 어느 id 가 어디로 합쳐졌는지의
유일한 기록이다.**

> **문서 ID 9개가 없어졌다.** 정합성 저장소의 manifest 에 그 id 가 적히면 `DMS에 없음` 으로
> 오분류된다. **사람이 저쪽에 알렸다 (2026-09-09)** — 이 항목은 닫혔다. 다만 저쪽이 실제로
> 반영했는지는 이 리포에서 알 수 없다. `python3 scripts/dms_sync.py` 리포트에 사라진 id 가
> `DMS에 없음` 으로 뜨면 아직 안 된 것이다. 합쳐진 id 대응은 `runs/merge-prod-20260909.json`.

> **첫 접속 문자열은 운영이 아니라 dev 였다.** 적용 전 대상 확인(활성 34건·`정책결정관리서`
> 2건)이 잡았다. 그대로 돌렸으면 전 그룹이 "버전이 1개가 아님"으로 건너뛰어져 **성공한 것처럼
> 보이는데 운영은 그대로**가 됐을 것이다. **운영 스크립트는 실행 전 대상 확인을 붙일 것.**

**미완**: 붙이기 기능이 없어서 **새 판은 여전히 별개 문서로 올라온다.** 방금 합친 것이
다시 갈라지기 시작한다 — 아래 "다음 작업" 0번이 그것이다.
...
**계획된 작업이 하나 있다 — 아래 0 번.** 나머지(1·2·3)는 코드가 아니거나 계기 대기다.
M0~M6 과 이 목록의 예전 0~4 번은 전부 끝났고, 그 기록은 `HANDOFF-ARCHIVE.md` 의
"§다음 작업 — 끝난 항목들" 절에 있다.

0. **업로드 때 "새 판으로 붙일까, 새 문서로 올릴까" 를 묻는다** (사람이 지시함, 2026-09-08).

   **왜 급한가**: 2026-09-09 에 갈라진 7그룹을 운영에서 합쳤는데(위 "현재 위치"),
   **이걸 안 만들면 그 상태가 그대로 다시 갈라진다.** 메인 업로드는 원리상 항상 새 문서를
   만들기 때문이다. 합치기는 되돌릴 수 없는 작업이라 두 번 하고 싶지 않다.

   **이미 있는 것 — 다시 만들지 말 것.** 판정은 끝났다.
   - `src/lib/similar-document.ts` — `findSimilarDocuments()` 가 같은 폴더·같은 키·
     **내 문서만** 후보로 준다. 판번호 내림차순이라 **첫 원소가 현재 최신**이다.
     `similarCandidateQuery()` + `toSimilarCandidates()` 가 조회까지 짝으로 있다
     (휴지통 필터가 그 안에 있다 — 호출부에 맡기지 말 것)
   - `src/lib/file-version.ts` — `compareFileVersions()`. **한쪽이라도 판번호가 없으면
     `null`** 이다. 0(같음)으로 뭉개면 근거 없는 파일이 조용히 통과한다
   - 테스트 33건. 운영 실데이터 대조로 **오탐 0**

   **남은 것**: ① 업로드 다이얼로그 UI ② 서버 연결(붙이기를 고르면
   `POST /api/documents/{id}/versions` 로) ③ **판번호 경고**.

   **확정된 결정 (뒤집지 말 것)**
   - **기본값은 "새 문서"다.** 붙이기는 되돌릴 수 없다 — 붙인 판을 떼는 화면이 없다
     (버전 롤백은 명시적 범위 밖). 새 문서로 잘못 가는 것은 나중에 붙이면 되고,
     잘못 붙는 것은 방법이 없다. **비대칭이 이 결정의 전부다**
   - **남이 올린 문서에는 제안을 안 띄운다.** 새 버전 올리기는 올린 사람·관리자만 할 수
     있어서(2026-09-07) 눌러도 403 이다. `findSimilarDocuments` 가 이미 걸러 준다
   - **판번호 경고를 같은 스트림에 넣는다.** 아래 함정 때문이다

   **함정 — 재현까지 해 뒀다 (2026-09-08).** 옛 판을 나중에 올리면 **세 경로 전부 옛 판을
   최신이라고 답한다**: `GET /api/documents` · 다운로드(`?v=` 미지정) · 목록의 버전 열.
   전부 `versionNo desc` 로 최신을 고르기 때문이다. **에러가 안 난다** — 유일한 단서는
   목록의 버전 숫자가 거꾸로 가는 것뿐이다. 붙이기는 재업로드를 **주력 경로로 만드는 일**이라
   지금 거의 안 나는 이 사고를 상시로 끌어올린다. 그래서 경고를 나눠 넣으면 안 된다.

   **접근 방법은 미확정 — 구현 전에 사람에게 확인할 것.** 후보 문서를 업로드 다이얼로그가
   어떻게 아느냐가 갈린다. 서버 컴포넌트가 prop 으로 내려주는 쪽(추가 API 없음, 25건이라
   페이로드 무시 가능)이 싸 보이지만 정하지 않았다.

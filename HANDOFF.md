...
**다음은 0-1(제안 이름 편집)이고, 그다음이 M5(미리보기)다.** 근거는 "다음 작업"에 있다.

> **다음 세션은 여기서 시작한다 — 0-1 을 파이프라인으로 돌린다.**
> `origin/main` 은 `f31bce0` 까지 나가 있고 0-2 는 운영에 반영됐다.
> 손대기 전에 읽을 것: 아래 "다음 작업" 의 0-1 항목(사양과 조건) · `MILESTONES.md`
> 자동 분류 절의 "실측에서 드러난 것"(표와 숫자) · `feature/auto-classify` worktree 의
> `.pipeline/auto-classify/DESIGN.md` §4·§6.
> **왜 이건 파이프라인이 맞는가**는 위 "실측 방법" 절 1번의 인용 블록에 있다 —
> 0-2 를 손으로 한 것과 정확히 반대되는 근거다.
> **주행 중에 셀 것 하나**: 권한 거부 건수. 0-3 으로 `PowerShell(...)` 을 넣었으므로
> 3차 주행의 8건에서 **4건으로 떨어지면 그 조치가 실증된다.** 안 떨어지면 규칙 이름이
> 안 맞은 것이니 `.claude/settings.local.json` 을 다시 본다.

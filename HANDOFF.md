...
0-1. **자동 분류의 제안 폴더 이름을 고친다** (2026-08-26 실측에서 드러남). 제안이
   카테고리가 아니라 문서 제목으로 나오고(`건강기록 와이어프레임 HLT`), 문서 1건짜리
   폴더가 파일 수만큼 생긴다. **파일명만으로는 카테고리와 제목을 구분할 신호가 없어
   정규식으로는 못 푼다.** 권장: (A) 2건 이상이 같은 이름을 제안할 때만 폴더 제안 +
   (B) 미리보기에서 제안 이름 편집. 근거와 표는 `MILESTONES.md` 자동 분류 절의
   "실측에서 드러난 것".

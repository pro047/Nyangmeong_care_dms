...
> **⚠ 지금 열려 있는 작업이 하나 있다 — 폴더 2뎁스 자동 분류.** worktree
> `../dms-folder-depth2` 에서 **파이프라인이 완주하고 검증도 통과했다**(2026-09-02).
> 남은 것은 **브라우저·DB 실측**이고, **main 머지는 안 했다.** 절차는 아래
> §'폴더 2뎁스 자동 분류 스트림'. 다른 일을 먼저 하려면 그 worktree 를 건드리지 말 것.

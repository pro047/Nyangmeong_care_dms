...
| 휴지통 행 버튼이 세로로 쪼개진다 | **2026-08-27 관찰, 원인 미확인.** 0-2 스크린샷(`DC2-purge.png`, 1440×780)에서 `복구`·`영구삭제` 글자가 한 글자씩 세로로 쌓이고 행 높이가 비정상으로 커져 있다. `전체 문서`(DC1)는 멀쩡하고 **휴지통만** 그렇다. 0-2 와 무관하고 `trash-row-actions.tsx` 는 이번에 손대지 않았으므로 **기존 결함으로 보인다.** 계기가 오면 볼 것 |

...
전부 닫혔고, 그 뒤에 연 스트림 일곱(xlsx·html 미리보기 · S3 고아 정리 · 폴더 2뎁스 ·
구버전 흐리기 · 목록 버전 열 · 권한 경계 · **최신 판정 컬럼**)도 **전부 배포까지 끝났다.**
...
### 최신 판정 컬럼 (2026-09-08) — ✅ 배포·운영 확인까지 끝

**팀이 쓰다가 잡은 첫 결함이다.** 관찰 단계에서 나온 신호라는 점이 값이 있다 — 이 스트림은
백로그에서 고른 것이 아니라 사람이 화면에서 이상을 보고 신고한 것이다.

**증상** — 같은 폴더의 v0.4 제목을 고치자 **더 새 판인 v0.5 쪽에 구버전 표시가 붙었다.**

**원인** — 구버전 판정이 `Document.updatedAt` 을 봤다. `@updatedAt` 이라 제목·설명·폴더를
고치기만 해도 갱신되는 값이고, **파일이 새 것인지와 무관하다.** 목록 정렬도 같은 컬럼이라
판정만 고쳤다면 **흐려진 구버전이 선명한 최신본보다 위에 놓이는** 상태가 남았을 것이다.

**조치** — 판정·정렬·표시 셋을 전부 `createdAt` 으로 맞췄다. 두 규칙을
`documentListOrderBy()`·`latestCandidateQuery()` 로 `src/lib/latest.ts` 한 파일에 모으고,
표의 날짜 열을 `수정` → `올린 날짜` 로 옮겼다. 커밋 `c6bb32f`(판정) · `aea4631`(정렬·열).
상세는 `MILESTONES.md` §구버전 문서 흐리게 → "후속".

**운영 확인 완료** (2026-09-08, 사람이 화면으로) — v0.5 최상단·선명, v0.4 흐림, 날짜 열이
업로드 시각. **반증까지 돌렸다**: v0.5 제목을 고쳐도 순서·날짜·흐림이 안 움직였다.

> **여기서 배울 것 — 같은 컬럼이라도 화면마다 묻는 질문이 다르다.** `updatedAt` 을 목록
> 정렬에 쓴 것은 "최근 수정순"이 의도였으므로 **맞았다.** 틀린 것은 그 값을 *"어느 쪽이 새
> 판인가"* 에까지 재사용한 쪽이다. `route.ts` 의 PATCH 주석은 *"@updatedAt 이 갱신돼 목록
> 위로 올라온다 — 의도한 동작이다"* 라고 적고 있었는데, **그 주석은 자기 화면만 보고 있었다.**
> 값을 공유하는 두 번째 소비자가 생기는 순간 주석이 낡는다.

> **남은 한계 (알고 넘어간다).** `createdAt` 은 "나중에 올린 게 새 판"을 전제한다 — v0.5 를
> 올린 뒤 빠뜨렸던 v0.4 를 뒤늦게 올리면 여전히 뒤집힌다. 정확히 하려면 파일명 버전 표기
> (`fileVersionLabel`, `src/lib/file-version.ts`)를 비교해야 하고, 그건 아래 §다음 작업의
> "진짜 원인"(업로드 병합)과 같이 갈 크기다. **혼자 떼어 하지 말 것.**

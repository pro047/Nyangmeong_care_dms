...
**다음 스트림은 정해지지 않았다.** §다음 작업에 살아 있는 코드 작업이 없다 — 검색(관찰이
근거를 열어 둔 것)과 MCP 2단계(0-3)가 후보다.
...
**MCP 서버 1단계**가 다른 스트림으로 들어와 있다(`c2e658d`, 코드 완료·미배포).
기록은 `MILESTONES.md` §'MCP 서버 · 1단계 코드 완료'.
...
0-3(MCP 2단계)은 사람 결정 대기이고, 1~3 은 코드가 아니거나 계기 대기다. M0~M6 과 이

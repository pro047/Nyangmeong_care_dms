...
>
> **그 뒤 `feature/xlsx-preview` 에서 html·xlsx 미리보기가 끝났다** (2026-08-28,
> 브라우저 실측 10/10 + 팀 xlsx 10건 전수 렌더 확인). **아직 main 에 안 들어갔다.**
> 이걸로 활성 문서 28건 중 미리보기가 되는 것이 3건 → **28건 전부**가 된다.
> 상세는 아래 "다음 작업" 3번.
>
...
3. **xlsx 미리보기** — ✅ **완료 (2026-08-28), 브라우저 실측 통과.**
   ...
   **남은 것.** 운영 오리진에서의 CORS 확인(배포 후). docx 는 여전히 범위 밖이다.

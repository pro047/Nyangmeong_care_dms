...
우선순위 순 (2026-08-24 확정). M3 는 같은 날 끝났다 (브라우저 실측 포함).

**1번을 먼저 하는 이유**: 2번을 파이프라인으로 돌릴 텐데, 그 파이프라인이 죽으면 지금은
사인이 한 줄도 안 남는다. 도구를 먼저 고쳐야 그 도구로 다음 작업을 돌릴 수 있다.

1. **파이프라인 수정** — 지시서는 리포 루트의 `PIPELINE-FIX.md` 에 있다. 결함 3건(아래 참조)을
   `orchestrate.sh` 에서 고치고 `test/fake-claude` 시나리오로 검증한다. **손으로 고칠 것** —
   `./orchestrate.sh` 로 돌리면 실행 중인 셸을 그 실행이 수정하게 된다. 끝나면 그 파일은 지운다.
...
**파이프라인 자체에 남은 결함 3건** (2026-08-24 하루에 세 번 죽으며 드러났다).
고칠 내용은 위 1번 — 리포 루트의 `PIPELINE-FIX.md`:

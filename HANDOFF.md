...
**계획된 코드 작업이 없다 — 배포 하나가 열려 있다.** (2026-09-11 추가: **MCP 서버
스트림이 설계까지 갔다** — 아래 "다음 작업" 0-3. 사람 결정 3건이 나야 코드에 들어간다.)
...
0-3. **MCP 서버 — 설계 완료 · 결정 3건 확정 · 구현 착수 가능 (2026-09-11).** Claude Code·
   Codex·claude.ai·ChatGPT 가 문서를 찾아 읽고 새 판으로 올리게 한다. 설계 전문은
   `MILESTONES.md` §'MCP 서버 · 설계 완료', 확정 근거는 같은 파일 '확정된 설계 결정' 표의
   MCP 행 3개(① 패키지 추가 ② 무상태 JWT ③ 파일은 URL 만·샌드박스 A 먼저 실측).
   **1단계(OAuth + 읽기 도구 4개)부터** 간다. 브랜치 `feature/mcp`.
   **원격 `main` 에는 MCP 코드가 없다** — 2026-09-11 `git grep` 전수 확인.

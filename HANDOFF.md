...
**계획된 코드 작업이 없다 — 배포 하나가 열려 있다.** (2026-09-12 갱신: **MCP 서버 1단계는
배포됐고 운영에서 ⓪ⓐ①ⓒⓓ✚ 가 통과했다** — 아래 "다음 작업" 0-3. 남은 것은 ⓑⓔⓕⓖ 와 2단계.)
...
**살아 있는 코드 작업은 MCP 2단계 하나다** (2026-09-12). 1단계가 배포·확인까지 갔으므로
다음은 0-3 의 2단계(`upload-commit.ts` 추출 + 올리기 도구 5개)다. 0 번(붙이기)은 코드가
끝났고 배포만 남았고, 0-1 은 닫혔다. 0-2 는 급하지 않은 사람 결정이다.
...
*이 문단은 2026-09-11 까지 "살아 있는 코드 작업이 없다"로 끝났는데, 그 뒤 MCP 가
1단계까지 갔다 (2026-09-12 정정).* **다음 코드 작업은 MCP 2단계**이고, 그것이 끝난 뒤의
스트림을 §운영 실측이 연 근거(팀이 쓴다)로 고른다.

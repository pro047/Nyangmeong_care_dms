...
**업로드가 자동으로 새 버전이 되고 목록이 최근 업로드순이 됐다 — 코드 완료 · dev 검증됨 ·
미배포** (2026-09-22, 브랜치 `feature/auto-attach-recent-upload`). 팀원이 개정할 때마다 새 문서로
...
### 자동 새 버전 · 최근 업로드 (2026-09-22) — **코드 완료 · dev 검증됨 · 미배포**

브랜치 `feature/auto-attach-recent-upload` (main `c128e4d` 에서 분기). **푸시하면 운영 배포다.**
...
**회신은 안 보냈다** — 사람이 "이제 여기서 작업한다"고 정했다. 인계문 §10 의 판단 1~4 는 **아직
사람 결정 전**이다. 세션 안(근거 등급 표시):

1. 서버가 S3 본문을 읽고 html 을 파싱하는 것 — **받아들일 수 있다고 본다.** 이미 MCP 가 한다
   (`getObjectBytes` `src/lib/s3.ts:63` · `htmlToText` `src/lib/mcp/html-text.ts:28`, 코드 확인).
   다만 `CLAUDE.md` 의 "읽기는 예외다(MCP 한정)" 조항을 넓히는 일이다 — 보호 조항 변경
2. docKey — **컬럼 + `@unique`** 권고. 태그는 한 태그에 문서가 여럿 붙는 구조(`DocumentTag`)라 DB 가
   유일성을 못 보장한다. 스키마 변경
3. html 파서 — 패키지 추가라 결정 대기. 후보 `htmlparser2`·`parse5`(가볍고 스크립트 실행 없음) vs `jsdom`
4. 정의 ID 조회 경로 — 필수 아님, 미루기 권고
...
**살아 있는 코드 작업: 자동 새 버전 · 최근 업로드 (2026-09-22, 미배포)** — 위 §현재 위치. 그다음이
**정합성 검사 이관 + docKey + 합치기**다(같은 절). 아래 문단은 2026-09-14 기준이다.

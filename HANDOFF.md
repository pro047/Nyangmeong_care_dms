...
**정합성 지표 화면이 코드 완료·dev 검증까지 끝났다** (2026-09-13). 메인 문서 목록 위에
밴드로 붙였고 축 9개·findings 149건이 요건대로 나온다(E2E 11/11). **운영에는 아직
안 밀었다** — `measured_at` 타입 변경과 배포가 사람 승인 대기다. 일지는 아래 절,
남은 절차는 `MILESTONES.md` §'정합성 지표 표시' 의 *남은 것*.

**버전 이력 표시가 끝났다** (2026-09-13). 실사용 신고에서 나왔다 — 새 버전을 올렸는데
목록이 안 바뀌어 *"이전 문서가 하나도 없다"* 로 읽혔다(실제로는 5회가 쌓여 있었다).
목록에 `v0.3 · 3회` 신호를 달고 눌러서 펼치게 했다. **화면 문구에서 "판"을 전부 뺐다** —
같은 동작을 상세는 `새 버전 올리기`, 업로드는 `새 판으로 붙이기` 로 부르고 있었다.
기록은 `MILESTONES.md` §'버전 이력 표시'.

**MCP 서버는 2단계까지 배포됐다** (1단계 2026-09-12 · 2단계 2026-09-13) — 읽기 도구 4개 +
올리기 도구 5개가 운영에서 돈다. 다음은 3단계(웹챗에서 문서 읽기)이고 B안(서버가 텍스트로
싣기)으로 확정됐다. 전부 다른 세션이 올린 스트림이다 — 기록은 `MILESTONES.md`
§'MCP 서버 · 1~2단계'.

**다음 스트림 후보는 둘이다.** ① MCP 2단계(올리기 도구 — 0-3) ② 검색(관찰이 근거를 열어
뒀다). 정합성 화면의 운영 반영은 스트림이 아니라 **이 스트림의 남은 절차**다.
...
**MCP 서버는 2단계까지 배포됐다** (1단계 `c2e658d` 2026-09-12 · 2단계 `f04dd12` 2026-09-13).
읽기 4개 + 올리기 5개가 운영에서 돈다 — 요구사항정의서 v5 · AI매니저/공통레이아웃
화면설계서 v3 을 **실제로 MCP 로 올렸다**. **다음 MCP 작업은 3단계 — claude.ai 웹챗에서 문서를
바로 읽어 분석하게 하기**. **B안(서버가 텍스트로 싣기)으로 확정**(사람 결정 2026-09-13) — 착수 지시는 아래 0-3. 정합성 지표 화면
(0 번)은 메인 체크아웃에서 따로 진행 중이다 — **두 스트림이 3002 포트를 같이 쓴다.**
...
### 정합성 지표 화면 (2026-09-13) — **코드 완료 · dev 검증됨 · 운영 미반영**

메인(`/`) 문서 목록 **위**에 밴드 하나. 배치와 타입 두 결정은 사람이 정했다(2026-09-13) —
근거는 `MILESTONES.md` §'확정된 설계 결정' 의 *정합성 지표 표시* · *`measured_at` 을
`timestamptz` 로* 행, 주행 기록과 실측표는 같은 파일 §'정합성 지표 표시'.

**축 9개는 항상 펼쳐 두고 findings 149건만 접는다.** 접힘 상태에서 한 축만 보이면 그게
*"정합성 = N%"* 라는 판정이 되는데 그 판정은 팀장이 한다.

**저쪽 완료 판정이 닫혔다** — *"메인에서 축 9개와 findings 149건이 보인다"*. 다만 **dev
에서만** 이다. 운영 반영 전까지 저쪽에 완료로 알리지 말 것.

**실측**: 메인 응답 503ms(스냅샷 있음) vs 508ms(없음) — 차이 없다. HTML 은 +42.3KB 인데
**gzip 1.6KB** 다. 밴드는 접힘 기준 약 215px 를 쓴다 — 문서 표가 그만큼 밀린다.

> **착수 지시서의 함정 하나가 실측으로 뒤집혔다.** *"`measuredAt` 을 그대로 찍으면 9시간
> 틀린다"* 를 전제로 `timestamptz` 를 골랐는데, **`timestamp` 로 되돌려 돌린 E2E 가 11/11
> 그대로 통과했다.** 쓰는 쪽과 읽는 쪽이 같은 TZ 면 오차가 상쇄된다. 9시간이 실제로 드러나는
> 것은 **다른 경로로 읽을 때**다 — `::text`·psql·`toISOString()`. 타입 변경은 유지하되
> (그 모호함이 2026-08-26 오판의 원인이었다) **"화면 버그를 고쳤다"고 적지 말 것.**

**대시보드로 다시 그렸고 한국어로 바꿨다** (2026-09-13, 사람 지시). 첫 판이 `FN→REQ 42/42
100.00%` 로 찍혀 *"영어랑 화살표로만 되어 있어 이해할 수 없다"* 는 지적을 받았다 —
**`CLAUDE.md` 의 "UI 문구는 한국어"를 어긴 것**이다. 구조도 "축 9개"에서 **팀원이 묻는 질문
셋**으로 바꿨다. 표와 근거는 `MILESTONES.md` §'대시보드로 다시 그림'.

> **저쪽에서 계약이 하나 더 온다 — 아직 안 받았다** (2026-09-13 회신). 새 축
> `scrFuncCoverage` 가 **문서별로 9개** 오고, **`to` 에 문서 종류가 아니라 문서 키**
> (`SCR-CMU`)가 들어온다. `findings 149 → 269` · `metrics 9 → 18` · 페이로드
> `25.4KB → 52.1KB`. 저쪽이 *"문서별 목록으로 그려 달라 — 합치면 편차가 사라진다"* 고
> 명시했다(`0/4` 부터 `98/98` 까지 있다).
>
> **이 축이 오면 지금 코드가 뭉갠다 (실측 확인).** `axisGroups` 가 `reference` 일 때만
> key 에 from/to 를 넣어서, 같은 축의 문서별 행 4개가 **key 1개로 합쳐진다.**
> `toView` 의 key 규칙을 고쳐야 한다.

**코드 리뷰가 7건을 냈고 전부 고쳤다** (`/code-review high`). 파이프라인의 `verify` 자리를
이걸로 대신했다. 표는 `MILESTONES.md` §'정합성 지표 표시' 의 *코드 리뷰*. 가장 큰 것은
**클라이언트 컴포넌트에서 `formatRelative` 를 부른 것** — 하이드레이션 날짜가 갈린다.

> **`format.ts` 의 기존 결함도 같이 닫았다** (사람이 정함, 2026-09-13). `formatRelative`
> 의 7일 초과 분기가 `toLocaleDateString` 에 **`timeZone` 을 안 박고 있었다**. 운영이 UTC 라
> **문서 목록·휴지통에서 한국 시각 새벽 0~9시에 올린 문서가 하루 이르게** 표시됐다. 이번
> 패널은 그 함수를 안 쓰는 쪽으로 피했지만 기존 두 화면이 남아 있었다. **이 스트림이 건드린
> 화면 밖의 수정이다** — 목록·휴지통의 오래된 날짜 표시가 바뀐다.

> **타임존 없는 컬럼을 키로 쓴 delete 가 조용히 0행을 지웠다.** E2E 정리가
> `delete where measured_at = $1` 이었는데 **0행을 지우고 "0건 남음"으로 보고**했다 —
> 확인 쿼리가 같은 어긋난 조건이라 자기모순이 없었다. 다음 주행이 FAIL 로 드러냈다.
> 지금은 POST 응답의 id 로 지운다. **자기 정리를 자기 조건으로 검산하면 못 잡는다.**
...
**0 번(정합성 지표 화면)은 코드가 끝났다 — 남은 것은 운영 반영뿐이다** (2026-09-13).
착수 지시서 전문은 `HANDOFF-ARCHIVE.md` 의 "§정합성 지표 화면 착수 지시서" 절로 내렸다.
0-3(MCP)은 **2단계까지 배포됐고** 3단계가 남았다. 1~3 은 코드가 아니거나 계기 대기다. M0~M6 과 이
목록의 예전 0~4 번은 전부 끝났고, 그 기록은 `HANDOFF-ARCHIVE.md` 의 "§다음 작업 — 끝난
항목들" 절에 있다.

0. **정합성 지표 화면 — 운영 반영만 남았다** (사람 승인 필요, 2026-09-13).

   dev 는 끝났다(E2E 11/11). 운영에 `measured_at` 타입 변경 + 배포가 남았고 **되돌리기
   어렵다.** 절차 4단계는 `MILESTONES.md` §'정합성 지표 표시' 의 *남은 것* 에 있다.
   핵심 둘: ① `ALTER ... USING measured_at AT TIME ZONE 'UTC'` 에서 **`USING` 을 빼지 말 것**
   (Prisma 생성 SQL 에는 없다) ② 운영에 **진짜 측정 1건이 들어 있으므로** 변환 후
   `2026-09-11 11:14:48+00` 인지 대조할 것.

   **저쪽에 완료로 알리는 것은 배포 뒤다** — 저쪽 완료 판정이 "메인에서 보인다"인데
   지금 보이는 곳은 dev 뿐이다.

0-3. **MCP 서버 — 1·2단계 배포됨 · 다음은 3단계 (2026-09-13).** 아래 본문은 1단계 시점에
   쓴 것이라 **2단계 이후 사정이 안 반영돼 있다** — 정본은 `MILESTONES.md`
   §'MCP 서버 · 1~2단계' 다. Claude Code·Codex·claude.ai·
   ...
   **남은 것**: ⓑ Codex · ⓔ 1시간 뒤 리프레시 · ⓖ 비길드 차단(부계정이 없어 미확인) ·
   ⓕ ③ A — **원인은 claude.ai 코드 실행 샌드박스의 네트워크 허용 목록이다**(2026-09-13 사람
   확인). 버킷 도메인을 허용 목록에 넣고 재시도한 결과는 **아직 없다**.
   **2단계는 배포됐다 (2026-09-13, PR #3 → `f04dd12`).** `upload-commit.ts` 추출 + 올리기
   도구 5개. 파이프라인 `mcp-upload` + `/code-review high` 6건 반영 + `/security-review` 0건.
   **확인 6개 통과**: 새 판 붙이기(요구사항정의서 v5·알림·화면) · 다운로드 무손실 ·
   안 된 파일 지우기 · 된 파일은 안 지우기 · 남의 문서 403(가짜 관리자 id 대조 실험) ·
   후보 거절. 표는 `MILESTONES.md` §'2단계 구현' 의 *배포와 사람 확인*.
   **실사용 업로드 (2026-09-13)**: 요구사항정의서 v5 · AI매니저/공통레이아웃 화면설계서 v3 —
   셋 다 내려받아 sha256 일치. `건강기록_화면설계서_v0.6_20260911` 은 **보류**(운영에 담당자의
   0912 판이 더 최신이다 — 붙이면 역행한다).
   **남은 것**: 화면 회귀 — 3002 가 풀리면 이 워크트리로 dev 를 띄우고 `npm run test:e2e:orphan`
   (X1·X2·X3 취소·X6~X9). **3002 의 `cwd` 부터 확인할 것** — 메인 체크아웃 서버는 2단계 전
   코드라 그대로 돌리면 거짓 초록불이다.

   **다음 세션 착수 — 3단계: 웹챗에서 문서를 바로 읽기 · B안 확정** (사람 결정 2026-09-13).
   막힌 곳은 claude.ai 샌드박스의 네트워크 허용 목록이다(위 ⓕ). **허용 목록을 고치는 A 는 하지
   않는다** — 사용자별 설정이라(추정) 팀원 7명이 각자 바꿔야 하기 때문이다. 대신 **서버가 S3 에서
   파일을 받아 텍스트로 바꿔 도구 응답에 싣는다.** 근거는 `MILESTONES.md` 설계 결정 표
   '웹챗 읽기' 행. 착수 순서:
   1. **범위는 정해졌다** (사람 결정 2026-09-13) — ⓐ **형식**: 텍스트(html·md·csv·txt) + xlsx(시트별 행).
   ...
   2. **xlsx 파서는 새로 들이지 않는다** — `exceljs 4.4.0` 이 이미 의존성이다(`package.json:35`).
      ...
      *이 항목은 원래 "패키지 추가 게이트" 로 적었는데 확인해 보니 틀렸다 — 2026-09-13 정정*
...
배포까지 갔고 정합성 지표 수신 API 도 들어왔다 (2026-09-12 정정).* **0 번(정합성 지표
화면)은 2026-09-13 에 코드가 끝났다** — 남은 것은 운영 반영이고 그건 스트림이 아니다.
그 다음은 **MCP 2단계**(0-3)와 **검색**(§운영 실측이 연 근거 — 팀이 쓴다) 둘 중에서 고른다.

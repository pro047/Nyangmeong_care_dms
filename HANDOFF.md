...
3. **xlsx 미리보기** (견적: 파이프라인 3회 $27~42 + 사람 5~8h). 사람이 요청했다 —
   "앞으로 xlsx 업로드가 많아질 것 같다". `MILESTONES.md:30` 의 확정된 설계 결정
   ("docx/xlsx는 서버 변환이 필요해 주말 범위 밖")을 뒤집는 일이고, **근거도 바뀐다** —
   서버 변환이 아니라 **브라우저 파싱**으로 가므로 원래 결정의 전제가 성립하지 않는다.
   착수 전에 표를 고치고 사유를 적을 것.

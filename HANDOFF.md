...
**고쳤다 (2026-09-14, PR #7 → `c7e1a68`).** `src/lib/xlsx-load.ts` 가 ExcelJS 로 먼저 열고
**실패할 때만** zip 을 풀어 접두사를 걷고 한 번 더 연다 — MCP `parseXlsx` 와 웹 미리보기가 같은
함수를 쓴다(`jszip 3.10.1` 직접 의존성, 사람 승인). 배포 뒤 운영 MCP 로 3건 모두 읽혔고(실측)
웹 미리보기도 열렸다(사람 확인). **xlsx 를 여는 새 경로를 만들면 `loadXlsx` 를 거칠 것** —

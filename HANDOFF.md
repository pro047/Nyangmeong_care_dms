...
**MCP 서버는 3단계까지 배포됐다** (1단계 `c2e658d` 2026-09-12 · 2단계 `f04dd12` 2026-09-13 ·
3단계 `88fa444` 2026-09-14). 읽기 4개 + 올리기 5개 + **웹챗에서 문서 본문을 읽는
`read_document`** 가 운영에서 돈다(B안 — 서버가 텍스트로 싣기, 사람 결정 2026-09-13).
전부 워크트리 `pearlfish`(`feature/mcp-read`) 세션이 올린 스트림이다 — **MCP 절의 상세는
그쪽이 맡는다.** 기록은 `MILESTONES.md` §'MCP 서버'.
...
**다음 스트림은 안 정해졌다.** 후보 둘: ① **MCP 3단계**(0-3, 착수 지시 있음)
② **검색**(§운영 실측이 근거를 열어 뒀다 — 팀이 쓴다).
...
**xlsx 3건이 `<x:workbook>` 네임스페이스 접두사라 ExcelJS 가 못 읽는다** (2026-09-14, MCP
워크트리 세션 실측). 운영 19건 중 3건이고 전부 `sannabi1` 업로드다. **웹 미리보기도 같은
로더(`xlsx-view.ts` → ExcelJS 4.4.0)를 쓰므로 그 3건은 미리보기가 안 열릴 가능성이 크다 —
아직 확인 안 했다(추정).** MCP 쪽에서 접두사 폴백 로더를 만들어 **웹 미리보기와 공유**하는
수정이 진행 중이다(`feature/mcp-read`). 그 PR 이 오면 미리보기도 같이 고쳐진다.
...
배포되어 닫혔고, **0-3(MCP)은 3단계까지 코드가 끝났다** — 남은 것은 배포와 사람 확인이다.

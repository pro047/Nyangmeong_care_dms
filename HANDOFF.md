...
1. **길드 비멤버 거부 확인 (M6 의 마지막 항목).** 다른 디스코드 계정이 필요하다.
   ...
   **`infra/s3-cors.json` 에 배포 주소를 넣는 것을 빠뜨리지 말 것** — 빠뜨리면 업로드가
   **조용히** 실패한다(브라우저 → S3 직접 PUT 이라 화면에 이유가 안 나온다).
   **이미 넣었다** (2026-08-25, `008b7da`). presigned PUT 이 실제로 통한 것이 그 증거다 —
   origin 이 어긋났으면 preflight 에서 막힌다.

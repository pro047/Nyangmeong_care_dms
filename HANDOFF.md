...
3. **xlsx 미리보기** — 진행 중. `feature/xlsx-preview` 브랜치.
   **착수 전 조건 셋은 전부 처리됐다** (2026-08-28): 설계 결정 표는 고쳤고
   (`MILESTONES.md` 미리보기 행), CORS 와 `numFmt` 는 실측했으며 **둘 다 결과가
   예상과 달라 설계가 줄었다**. **html 은 끝났다** (`c7eb035`). 남은 것은 xlsx 본체다.
   **아래 실측값을 다시 재지 말 것** — `.pipeline/` 을 잃은 것과 같은 이유로 여기 적어 둔다.

   **① CORS 는 문제가 아니다 — 라우트를 새로 만들 필요가 없다.**
   ...
   S3 체인을 Playwright 로 재현, 41897바이트 수신). S3 응답이
   `ACAO=http://localhost:3002` 였는데 — **대조로, curl 로 `Origin: null` 을 보내면
   S3 는 ACAO 를 안 준다.** 즉 브라우저가 `null` 이 아니라 진짜 오리진을 보냈다.
   same-origin 으로 시작한 fetch 는 response tainting 이 `basic` 이라 리다이렉트에서
   origin 이 강등되지 않는다. **운영 오리진도 S3 CORS 허용 목록에 있으므로 같이 통과할
   것이나, 그건 아직 안 봤다.**

   **② `numFmt` 엔진은 필요 없다 — 날짜 변환 하나면 된다.**
   원래는 "구현한다 / 빼고 날짜가 시리얼 숫자로 보이는 걸 받아들인다"의 양자택일로
   적혀 있었는데 **제3의 답이 나왔다.** 팀 xlsx 10개를 S3 에서 받아 깠더니 **값 있는
   숫자셀 549개 중 날짜가 43개(7.8%)이고 그중 41개가 `01_요구사항 정의서` 한 파일에
   몰려 있다.** 쓰이는 서식은 `yyyy-mm-dd` · `m/d/yyyy` · builtin 14 **3종뿐**이고
   통화·퍼센트·소수 서식 셀은 **0개**다. 게다가 **ExcelJS 가 시리얼을 이미 JS `Date`
   로 바꿔 준다** — 남은 일은 `Date` → 문자열이다.
   **`getUTC*` 를 쓸 것** — ExcelJS 가 주는 Date 는 UTC 자정이라 로컬 게터로 읽으면
   위 "DB 시각을 읽을 때 9시간이 밀린다"와 같은 자리를 밟는다.

   **③ 진짜 함정은 여기 있었다 — 채우기색이 `argb` 가 아니라 `theme` + `tint` 다.**
   원래 계획은 "ARGB→hex"였는데 팀 파일의 실제 값은
   `{"theme":8,"tint":0.7999...}` 다. **그리고 팀 파일의 테마 팔레트가 세 종류로
   갈린다** — Office 2013+(`44546A E7E6E6 5B9BD5…`) 5건, Office 2007(`1F497D EEECE1
   4F81BD…`) 4건, 변형 1건. **팔레트를 하드코딩하면 절반이 틀린 색으로 나온다.**
   다행히 **ExcelJS 가 `workbook._themes.theme1` 에 원본 XML 을 들고 있다** — 거기서
   `clrScheme` 을 뽑아 쓴다. **xlsx 의 `theme=` 인덱스는 XML 순서와 다르다** —
   0=lt1, 1=dk1, 2=lt2, 3=dk2, 4~9=accent1~6 로 앞의 두 쌍이 뒤집혀 있다.
   `tint` 는 ECMA-376 의 HLS 식(`tint>0 → L' = L(1-t) + t`)을 따른다.

   **④ 그 밖에 실측으로 확인한 API 모양.** 병합은 `ws.model.merges` 가
   `["C210:C211", …]` 로 준다(한 파일 최대 129개, 총 310개). 병합에 덮인 셀은
   `cell.type === ValueType.Merge` 로 건너뛴다. **시트가 최대 4개라 시트 탭이 필요하다.**
   `ws.rowCount` 는 빈 스타일 행까지 세므로(표지 시트가 4셀인데 38행) **값 기준으로
   실사용 범위를 다시 구해야 한다.** 열너비는 Excel 단위(143.7·8.7)라 px 환산이 필요하다.
   `bgColor`·테두리색의 `{indexed:64}` 는 시스템 기본이라 무시한다.

   **라이브러리는 ExcelJS 4.4.0(MIT) 이고 이미 설치했다.** npm 의 `xlsx`(SheetJS CE)는
   0.18.5 에 멈춰 있고(SheetJS 가 npm 을 떠났다) 그 뒤 고쳐진 프로토타입 오염·ReDoS
   수정이 없다. `xlsx-js-style` 은 그 0.18 포크라 같은 문제를 물려받고 2022-05 이후
   방치다. ExcelJS 는 `browser` 필드가 `dist/exceljs.min.js`(925KB / gzip 252KB)라
   동적 import 로 감당된다. **본체는 ExcelJS 연동이 아니라 서식→CSS 변환 레이어다** —
   순수 함수라 유닛 테스트로 덮이고, 그래서 **파이프라인 없이 직접 구현하기로 했다**
   (사람 결정, 2026-08-28).

   **남은 일.** `src/lib/xlsx-view.ts`(테마 파싱 · tint · 색 해석 · 열너비 환산 ·
   날짜 서식 · 병합 인덱스 — 전부 순수 함수 + 유닛 테스트) → `SpreadsheetPreview`
   클라이언트 컴포넌트(동적 import + fetch + 시트 탭) → `previewKind` 에 `'xlsx'` 추가 →
   브라우저 실측. 기준선은 이미 있다 — `npm run test:e2e:preview` 의 **H4 가 "xlsx 는
   아직 폴백 박스"를 단언**하므로, xlsx 가 붙으면 그 항목을 뒤집어 대조할 수 있다.

   **라이브러리는 사실상 ExcelJS 하나다** (2026-08-28 조사). npm 의 `xlsx`(SheetJS CE)는
   **0.18.5 에 멈춰 있고**(SheetJS 가 npm 을 떠나 자체 CDN 으로 갔다) 그 뒤 고쳐진
   프로토타입 오염·ReDoS 수정이 없다. `xlsx-js-style` 은 그 0.18 포크라 같은 문제를
   물려받고 2022-05 이후 방치다. **ExcelJS 4.4.0(MIT)** 은 fill·font·border·병합·열너비를
   읽고 브라우저 dist 가 925KB / gzip 252KB 라 동적 import 로 감당된다.

   **작업의 본체는 ExcelJS 연동이 아니라 서식→CSS 변환 레이어다.** SheetJS 의
   `sheet_to_html` 같은 출력 함수가 없어서 ARGB→hex, 병합→colspan/rowspan, 열너비 환산,
   그리고 **`numFmt`**(원시 값 + 포맷 문자열만 주므로 Excel 숫자 서식을 직접 구현해야
   한다)를 짜야 한다. numFmt 를 빼면 파이프라인 1회와 1~2h 가 줄지만 날짜가 시리얼
   숫자로 보인다 — **팀 xlsx 를 먼저 열어 보고 정할 것.**

   **먼저 실측할 것 하나** — 리다이렉트 뒤의 CORS. `fetch('/api/.../download?inline=1')`
   은 same-origin 으로 시작해 307 로 S3 에 넘어가는데, **교차 오리진 리다이렉트에서
   브라우저가 `Origin: null` 을 보내면** S3 CORS 의 오리진 목록에 걸려 거부된다.
   `<img>`·`<iframe>` 은 CORS 대상이 아니라 지금 미리보기가 이걸 안 밟았을 뿐이다.
   회피책은 **서명 URL 을 JSON 으로 돌려주는 라우트**를 두고 S3 를 직접 fetch 하는 것
   (파일은 여전히 앱 서버를 안 거치므로 불변식 유지). `infra/s3-cors.json` 은 이미
   `GET` 을 허용하므로 **인프라 변경은 없다.**

   **html 은 거의 공짜다** (코드 6줄 + 테스트 2케이스, 30분). 지금 iframe 이 이미
   S3 로 307 되고 `Content-Type: text/html` 이면 브라우저가 렌더한다. 실행 오리진이
   `*.amazonaws.com` 이라 앱 쿠키에 닿을 수 없다 — 이번 설계가 SVG 를 수용한 것과
   같은 논거다. **활성 문서의 다수가 `.html` 이라 체감이 가장 크다.**

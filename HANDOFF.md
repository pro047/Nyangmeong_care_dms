...
**MCP 서버는 2단계까지 배포됐고 3단계는 코드 완료·미배포다** (1단계 `c2e658d` 2026-09-12 ·
2단계 `f04dd12` 2026-09-13 · 3단계 `88fa444` 2026-09-14). 읽기 4개 + 올리기 5개가 운영에서
돌고, 3단계는 **웹챗에서 문서 본문을 읽는 `read_document`** 다(B안 — 서버가 텍스트로 싣기,
사람 결정 2026-09-13). 전부 다른 세션이 올린 스트림이다 — 기록은 `MILESTONES.md`
§'MCP 서버 · 1~3단계'.

> **3단계는 `main` 에 있지만 아직 운영에 안 갔다.** 즉 **다음에 `main` 을 푸시하는 사람이
> 3단계를 배포한다** — 정합성 화면 문구만 고치고 밀어도 같이 나간다. 배포 뒤 사람 확인이
> 필요하면 그쪽 세션과 순서를 맞출 것.

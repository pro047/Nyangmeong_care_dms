...
docx 미리보기 · 파일 내용 전문 검색 · 버전 롤백 버튼 · 문서 상태 라벨 ·
문서별 세부 권한 · 한 문서에 여러 파일 · 모바일 업로드

> 이 목록은 원래 **`docx/xlsx 미리보기`** 였다 (2026-09-13 정정). xlsx 미리보기는
> 2026-08-28 에 뒤집혀 배포까지 끝났다(`MILESTONES.md` 설계 결정 표 '미리보기' 행).
> MCP 읽기 도구가 문서 **한 건**을 텍스트로 돌려주는 것은 전문 검색(여러 문서의 내용을
> 색인해 찾기)이 아니다.

...
TEST_CMD="${TEST_CMD:-npm test && npm run lint && npm run build}"
...
PROTECTED_FILES="${PROTECTED_FILES:-package.json package-lock.json pnpm-lock.yaml yarn.lock pyproject.toml requirements.txt requirements-dev.txt setup.cfg pytest.ini mypy.ini ruff.toml .ruff.toml conftest.py tests/conftest.py vitest.config.ts vitest.config.mts jest.config.js jest.config.ts tsconfig.json pubspec.yaml pubspec.lock .gitignore CLAUDE.md AGENTS.md .env .env.example prisma/schema.prisma}"
...
AGENT_TOOLS="${AGENT_TOOLS:-Bash(git status:*),Bash(git diff:*),Bash(git log:*),Bash(git ls-files:*),Bash(npm test:*)}"
